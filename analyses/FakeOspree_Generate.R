...
summary(lme2 <- lmer(bb ~ (chill|sp) + (force|sp) +(photo|sp)+(lat|sp), data = fake)) 
...
################# New version, no lab group effects
...
# Basic model, almost right
summary(lm1 <- lm(bb ~ (chill+force+photo+lat)^2, data = fake)) # sanity check. 

# Fixed effects should be very close to coefficients used in simulating data
summary(lme1 <- lmer(bb ~ (chill+force+photo+lat)^2 + (1|sp), data = fake)) 
ranef(lme1)
data.frame(fixef(lme1))

# Can we get random slopes and intercept in lme? No.. and this doesn't even have interax.
summary(lme2 <- lmer(bb ~ (chill+force+photo+lat)^2 + (chill|sp) + (force|sp) +(photo|sp)+(lat|sp), data = fake)) 


AIC(lm1, lme1, lme2) # full mixed effect model much better, good!


save(list=c("fake"), file = "FakeOspreeNoLab.RData")
