...
preCC <- mst[(mst$year>=1960 & mst$year<=1970), ]
...
preCC<-preCC[(preCC$year>=1960 & preCC$year<=1970),]
...
mat$cc<-ifelse(mat$year>=1960 & mat$year<=1970, "apre", "post")
...
sites$siteslist<-1:25
tavg<-r

leaps<-c(1960, 1964, 1968, 2000, 2004, 2008)
...
predata<-data.frame(chillport = c(pre$Mean.Chill.1, pre$Mean.Chill.2,
                                 ...
                                 pre$Mean.Chill.17, pre$Mean.Chill.18,
                                 pre$Mean.Chill.19, pre$Mean.Chill.20,
                                 pre$Mean.Chill.21, pre$Mean.Chill.22,
                                 pre$Mean.Chill.23, pre$Mean.Chill.24,
                                 pre$Mean.Chill.25),
...
postdata<-data.frame(chillport = c(post$Mean.Chill.1, post$Mean.Chill.2,
                                  ...
                                  post$Mean.Chill.17, post$Mean.Chill.18,
                                  post$Mean.Chill.19, post$Mean.Chill.20,
                                  post$Mean.Chill.21, post$Mean.Chill.22,
                                  post$Mean.Chill.23, post$Mean.Chill.24,
                                  post$Mean.Chill.25),
...
## Combine and remove year 2000 since it's 1951-1960 in data
chillandforce<- full_join(allchills, mat)
#chillandforce <- chillandforce[!(chillandforce$year==2000),]

## Add in Utah data
chillandforce <- full_join(chillandforce, allchills)
chillandforce <- chillandforce[!(chillandforce$year==2000),]

#chillandforce <- read.csv("output/betpen_chillandforce.csv", header=TRUE)
############# Plotting Time! #################
xlab <- "Mean Spring Temperature (°C)"

quartz()
mat.plot<-ggplot(chillandforce, aes(x=mat, y=lo, col=cc)) + geom_line(aes(col=cc), stat="smooth", method="lm") + 
...
quartz()
chill.plot<-ggplot(chillandforce, aes(x=chillutah, y=lo, col=cc)) + geom_line(aes(col=cc), stat="smooth", method="lm") + 
  theme_classic() + labs(x=xlab, y="Day of Leafout") + theme(legend.position="none") +
  scale_color_manual(name="Years", values=c(apre = "darkblue", 
                                            post = "darkred"),
                     labels=c(apre = "1950-1960",
...
xlab <- "Total Chill Utah"

quartz()
chill.utah.plot<-ggplot(chillandforce, aes(x=chillutah, y=lo, col=cc)) + geom_line(aes(col=cc), stat="smooth", method="lm") + 
  theme_classic() + labs(x=xlab, y="Day of Leafout") + theme(legend.position="none") +
  scale_color_manual(name="Years", values=c(apre = "darkblue", 
                                            post = "darkred"),
                     labels=c(apre = "1950-1960",
...
quartz()
grid.arrange(mat.plot, chill.plot, chill.utah.plot, mylegend, ncol=4, widths=c(2, 2, 2, 0.75))





#write.csv(chillandforce, file="output/betpen_chillandforce.csv", row.names = FALSE)
