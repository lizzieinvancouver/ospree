...
d <- read.csv("output/ospree_clean_withchill.csv") # 30 Oct 2018: 12693 

# 2. Need to deal with thermal time to days
source("bb_analysis/cleaning/clean_thermaltimetodays.R") # 30 Oct 2018: 12693

# 3. Clean phenstage to get a little more data (a little, but still!). 
source("bb_analysis/cleaning/clean_respvarmore.R") # 30 Oct 2018: 12152 

# 4. Select out the highest percentage of budburst only, and remove studies that contain duplicate data in two forms
source("bb_analysis/cleaning/multiresp.R") # as of 16 July 2017, deletes ~2400 rows (same in May and Oct 2018) 9717 rows
...
source("bb_analysis/cleaning/clean_rampedandexpforcing.R")
...
source("bb_analysis/cleaning/clean_checkdateofclimatedata.R") # As of 30 Oct 2018: 9717 rows
...
source("bb_analysis/cleaning/clean_bbperctodays.R") # As of 12 Dec 2018: 7385 rows;  #as of 22 MAr 2019: 7645   rows

# 7. Clean duplicate responses across treatments/categories)
source("bb_analysis/cleaning/clean_moreduplicates.R") # As of 12 Dec 2018, deletes 2 rows (7383 ).#as of 22 MAr 2019 (7643)

# 8. Clean photoperiod entries to try to get as much data as possible
source("bb_analysis/cleaning/clean_photoperiod.R")

# 9. Add sample size and error to studies, if possible
source("bb_analysis/cleaning/clean_addnanderror.R")
