...
d <- read.csv("output/ospree_clean_withchill.csv") # 29 Apr 2018: 12693 (down from 12743 on 12 Apr)

# 2. Need to deal with thermal time to days
source("bb_analysis/cleaning/clean_thermaltimetodays.R") # 29 Apr 2018: 12693

# 3. Clean phenstage to get a little more data (a little, but still!). 
source("bb_analysis/cleaning/clean_respvarmore.R") # 29 Apr 2018: 12152 (12 Apr 2018: 12202)

# 4. Select out the highest percentage of budburst only, and remove studies that contain duplicate data in two forms
source("bb_analysis/cleaning/multiresp.R") # as of 16 July 2017, deletes ~2400 rows (same in Apr 2018) 9717 rows
...
source("bb_analysis/cleaning/clean_bbperctodays.R") # As of 29 apr 2018: 7536 rows (as of 25 apr 2018, deletes 1514 rows: 8253 rows)

# 7. Clean duplicate responses across treatments/categories)
source("bb_analysis/cleaning/clean_moreduplicates.R") # As of 29 Apr 2018, deletes 2 rows (7534).
...
write.csv(d, "output/ospree_clean_withchill_BB.csv", row.names=FALSE) ## As of 29 apr 2018: 7534 rows  (as of 12 Apr 2018: 8251)
