...
#print(i)
  ...
  dat1$treatment <- paste(dat1$genus,dat1$species, dat1$varetc,dat1$population,
      dat1$other.treatment, dat1$dormancy_induction_temp, 
      dat1$dormancy_induction_days, dat1$dormancy_induction_photoperiod_day,
      dat1$dormancy_induction_photoperiod_night,
      dat1$freeze.treatment.time, dat1$freeze.treatment.photoperiod_day,
      dat1$freeze.treatment.photoperiod_night,
      dat1$freeze.treatment.temp_day, dat1$freeze.treatment.temp_night,
      dat1$fieldchill,dat1$chilltemp, dat1$chillphotoperiod, dat1$chilldays,
      dat1$number.longdays, dat1$photoperiod_day,dat1$photoperiod_night,dat1$forcetemp,
      dat1$forcetemp_night, dat1$irradiance,dat1$humidity,
      dat1$fieldsample.date,sep = ".")
  ...
  for(j in 1:length(study)){ # j = 1
    ...
    for(k in 1:length(treat)) { # k = 6
    # This for loop cleans each treatment within studies with more than one treatment
    # print(paste(i,j,k))
    ...
    if(length(dat3$response[!is.na(dat3$response)])>1 & is.numeric(dat3$response.time))
      ...
      if(length(values.in.target)>1 & length(which(dat3$response==target.percent))==0){  
        index<-rownames(dat3[values.in.target,])  
        d[index,"dbb"]<-dat3[values.in.target,"response.time"]
        d[index,"maxperc_bb"]<-rep(max(dat3[values.in.target,"response"]),
            length(values.in.target))
        d[index,"minperc_bb"]<-rep(min(dat3[values.in.target,"response"]),
            length(values.in.target))
        d[index,"dist.50bb"]<-abs(dat3[values.in.target,"response"]-target.percent)
        
      }
      # If there are more than 1 rows with values within the acceptable range of target percent and at least one value in the 
...
if(type=="BB_analysis"){
  ...
  d.subsetted$response.time[which(!is.na(d.subsetted$dbb))] <-
      d.subsetted$dbb[which(!is.na(d.subsetted$dbb))]
  }
