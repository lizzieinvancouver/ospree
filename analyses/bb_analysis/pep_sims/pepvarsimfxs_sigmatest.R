...
pepvariance.sim.sigmatest <- function(drawstotal, yrstotal, precctemp, postcctemp, samplefreq, sigmapre, sigmapost, fstar){
draws <- c()
precc.sens <- c()
postcc.sens <- c()
var.lo.precc <- c()
...
for (i in 1:yrstotal){
    if (i<(yrstotal/2+1)) {
        dailytemphere <- rnorm(daysperyr, precc, sigmapre)
        } else {
        dailytemphere <- rnorm(daysperyr, postcc, sigmapost)
        }
