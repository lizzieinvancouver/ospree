...
par(xpd=FALSE)
par(mar=c(5,7,3,10))
plot(x=NULL,y=NULL, xlim=c(-9, 2), yaxt='n', ylim=c(0,3),
     xlab="Model estimate change in days to BB", ylab="", main="M1_daysBBnointer_2level")
...
par(xpd=TRUE) # so I can plot legend outside
legend(3, 3, sort(unique(bb.stan$complex.wname)), pch=my.pch[1:38], col=alpha(my.pal[1:38], alphahere),
   cex=0.75, bty="n")
}
