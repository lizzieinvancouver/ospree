...
if(use.onecue){
  bb.stan <- bb.stan.onecue
}

if(use.nocrops){
  bb.stan <- bb.stan.nocrops
  bb.stan$complex <- ifelse(bb.stan$complex==19,2,bb.stan$complex)
  bb.stan$complex <- ifelse(bb.stan$complex==20,10,bb.stan$complex)
  }

sort(unique(bb.stan.allphoto$complex.wname)) 
sort(unique(bb.stan$complex)) 

## subsetting data, preparing genus variable, removing NAs (err, again
# remove crops?
# bb.stan <- subset(bb.stan, type!="crop")
