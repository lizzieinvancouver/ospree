...
2. fieldchillcalc_latlong.R: Starts with ospree_clean.csv with cleaning chill temp column and pulls in climate data (from an external hard drive) using the provenance lat/long columns and calculates field chilling experienced before the material was collected for the experiment. Generates a new csv file with the just the chilling estimates for those studies with field sample dates: fieldchillcalcslatlong.csv
