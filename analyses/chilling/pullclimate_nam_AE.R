...
# now loop over these year-month combo files and get temperature values for this date range.
  ...
  for(j in c(yr)) { # j = yr
    ...
    tmax <- list.files(path=paste(climatedrive,nafiles, sep="/"), pattern=paste0("tmax_daily_",yr), full.names = TRUE)
    tmaxprev <- list.files(path=paste(climatedrive,nafiles, sep="/"), pattern=paste0("tmax_daily_",yr-1), full.names = TRUE)
    tmin <- list.files(path=paste(climatedrive,nafiles, sep="/"), pattern=paste0("tmin_daily_",yr), full.names = TRUE)
    tminprev <- list.files(path=paste(climatedrive,nafiles, sep="/"), pattern=paste0("tmin_daily_",yr-1), full.names = TRUE)
    jx <- nc_open(tmax)
    ...
    if(length(unique(maxtest))==1 & is.na(unique(maxtest)[1])){#if there are no temp data for the selected lat/long, choose a different one
     diff.long.cell[which(diff.long.cell==min(diff.long.cell,na.rm=TRUE))[1]]<-NA
      ...
      maxtestlastyr<-(ncvar_get(jx,start=c(long.cell,lat.cell,1),count=c(1,1,-1)))[lastyr]-273.15#check that the lat/long combinations has temperature data. 
      maxtest <- c(maxtestthisyr, maxtestlastyr)
      if(length(unique(maxtest))==1 & is.na(unique(maxtest)[1])){
        diff.long.cell[which(diff.long.cell==min(diff.long.cell,na.rm=TRUE))[1]]<-NA
        diff.long.cell[which(diff.long.cell==min(diff.long.cell,na.rm=TRUE))[1]]<-NA
        long.cell <- which(diff.long.cell==min(diff.long.cell,na.rm=TRUE))[1] #select the closest longitude & latitude with climate data to longitude[i]
        lat.cell <- which(diff.lat.cell==min(diff.lat.cell,na.rm=TRUE))[1]
