...
# NEXT! I need to check what is lots in conversion to numeric ...
...
osp.daynight.vary <- get.treatdists.daynight(datsm.noNA.daynight, "force", "forcenight")

dat$datasetIDstudy <- paste(dat$datasetID, dat$study)
ampstudies <- dat[which(dat$datasetIDstudy %in% paste(osp.daynight.vary$datasetID, osp.daynight.vary$study)),]
