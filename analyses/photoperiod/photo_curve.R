...
ggplot(hei,aes(as.numeric(photoperiod_day),response.time))+geom_line(aes(color=population))
...
ash<-subset(bbdat,datasetID=="ashby62")
...
ash1<-subset(ash1,response.time<999)
...
ggplot(ha,(aes(as.numeric(photoperiod_day),response.time)))+geom_line(aes(color=datasetID,group=thingy))
...
ha<-rbind(caf,hei,ash1,ash2)
ggplot(ha,(aes(as.numeric(photoperiod_day),response.time)))+geom_line(aes(color=datasetID,group=thingy))
ggplot(ha,(aes(as.numeric(photoperiod_day),response.time)))+geom_line(aes(color=datasetID,group=thingy))+facet_wrap(~datasetID)
