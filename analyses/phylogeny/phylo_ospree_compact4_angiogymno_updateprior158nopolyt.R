...
#'###################################
# Remove species in polytomies      ####
#'###################################

polyt.genus = c("Salix", "Fagus","Quercus","Sorbus","Rhamnus","Acer","Buddleja","Fraxinus")
d.polyt = subset(d, genus %in% polyt.genus )
to.rempolyt = table(d.polyt$latbi)
to.rempolytnames = names(to.rempolyt[-c(7,11,18,21,35,39,43,48)])

phylo.pol <- drop.tip(phylo, to.rempolytnames)
d.pol = subset(d, !latbi %in% to.rempolytnames)
phylo.pol
length(unique(d.pol$latbi))
d = d.pol
phylo = phylo.pol
...
if(runmodels){
  ...
  saveRDS(fit, "output/fit_priorupdate_noout_angio158_nopolyt.rds")
