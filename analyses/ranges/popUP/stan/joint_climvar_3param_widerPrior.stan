/* this stan code is similar to joint_climvar_3param_emw.stan but with a more reasonable prior for the intercept mu: NCP on photo and forcing */
...
parameters{
    ...
    //MODEL 2 -----------------------------------------------------
    //level 2
    vector[n_spec] Force_ncp; // new for NCP
    ...
    vector[n_spec] Photo_ncp; // new for NCP
...
transformed parameters{
    ...
    //NCP on photoperiod and forcing
    vector[n_spec] alphaPhotoSp; // moved down to transformed parameters for NCP
    vector[n_spec] alphaForcingSp; // moved down to transformed parameters for NCP
    alphaPhotoSp = muPhotoSp + sigmaPhotoSp*Photo_ncp;
    alphaForcingSp = muForceSp + sigmaForceSp*Force_ncp;
...
model{ 
    ...
    sigmaForceSp ~ normal(0, 20); // removed for NCP
    muForceSp ~ normal(0, 50);
    //alphaForcingSp ~ normal(muForceSp, sigmaForceSp);
    Force_ncp ~ normal(0, 10);
    
    sigmaPhotoSp ~ normal(0, 10);// prior for photoperiod
    muPhotoSp ~ normal(0, 10);
    Photo_ncp ~ normal(0, 10);
    //alphaPhotoSp ~ normal(muPhotoSp, sigmaPhotoSp); // removed for NCP
    
    sigmaChillSp ~ normal(0, 10);// prior for photoperiod
    muChillSp ~ normal(0, 50);
    alphaChillSp ~ normal(muChillSp, sigmaChillSp); 



   // sigmaPhenoSp ~ normal(0, 5); //priors for phenology 
    muPhenoSp ~ normal(60, 50);
