...
muplotfx <- function(modelhere, nameforfig, width, height, ylim, xlim, leg1, leg2){
  ...
  popnum <- unique(mod.ranefpop$pop[!is.na(mod.ranefpop$pop)])
  ...
  axis(2, at=2, labels=c("Forcing"), las=1)
  ...
  for(i in 1){#i=1
    ...
    for(spsi in 1:length(spnum)){#spsi=12
      ...
      for(i in 1:c(nrow(mod.ranefpop))){
        if(sps==mod.ranefpop$species[i]){
  ...
  legend(leg1, leg2, sort(unique(gsub("_", " ", df$species))), pch=19,
