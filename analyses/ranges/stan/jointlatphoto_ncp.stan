...
transformed parameters{
  ...
  vector[nstudy] a_study = mua_study + a_study_ncp * sigma_study; // mean of the alpha value for studies 
	...
	vector[nsppheno] a_photo = mua_photo + a_photo_ncp * sigma_bphoto;
	    ...
	    b_photo_final[isp] =  muaphoto[isp] + bphoto_lat * mua_sp[isp];
	}
}
...
model{ 
	...
	for (iph in 1:Npheno){
	ypredpheno[iph] = a_spheno[speciespheno[iph]] + b_photo_final[speciespheno[iph]] * photoperiod[iph];
        }
	// Model of lat
	a_sp_ncp ~ normal(0, 1);
