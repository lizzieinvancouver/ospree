...
lower_htBFSpMean <- as.numeric(round(HPDI(data.frame(htModelFit$betaTraitxForce), prob = 0.90)[1],1))
upper_htBFSpMean <- as.numeric(round(HPDI(data.frame(htModelFit$betaTraitxForce), prob = 0.90)[2],1))
...
lower_slaBCSpMean <- as.numeric(round(HPDI(data.frame(slaModelFit$betaTraitxChill), prob = 0.90)[1],1))
upper_slaBCSpMean <- as.numeric(round(HPDI(data.frame(slaModelFit$betaTraitxChill), prob = 0.90)[2],1))
...
lower_lncBCSpMean <- as.numeric(round(HPDI(data.frame(lncModelFit$betaTraitxChill), prob = 0.90)[1],1))
upper_lncBCSpMean <- as.numeric(round(HPDI(data.frame(lncModelFit$betaTraitxChill), prob = 0.90)[2],1))
...
lower_smBFSpMean <- as.numeric(round(HPDI(data.frame(smModelFit$betaTraitxForce), prob = 0.90)[1],1))
upper_smBFSpMean <- as.numeric(round(HPDI(data.frame(smModelFit$betaTraitxForce), prob = 0.90)[2],1))
smBFSpMean; lower_smBFSpMean; upper_smBFSpMean

smBCSpMean <- as.numeric(round(mean(smModelFit$betaTraitxChill),1))
lower_smBCSpMean <- as.numeric(round(HPDI(data.frame(smModelFit$betaTraitxChill), prob = 0.90)[1],1))
upper_smBCSpMean <- as.numeric(round(HPDI(data.frame(smModelFit$betaTraitxChill), prob = 0.90)[2],1))
...
lower_htMax<- as.numeric(round(HPDI( as.vector(longMeans$traitMean[longMeans$speciesname == "Quercus_petraea"]) , prob=0.90 )[2],1))
upper_htMax <- as.numeric(round(HPDI( as.vector(longMeans$traitMean[longMeans$speciesname == "Quercus_petraea"]) , prob=0.90 )[1],1))

htMin <- as.numeric(round(min(speciesMeans$traitMean)))
#Uncertainty around max species
lower_htMin<- as.numeric(round(HPDI( as.vector(longMeans$traitMean[longMeans$speciesname == "Rhamnus_cathartica"]) , prob=0.90 )[2],1))
upper_htMin <- as.numeric(round(HPDI( as.vector(longMeans$traitMean[longMeans$speciesname == "Rhamnus_cathartica"]) , prob=0.90 )[1],1))
...
#Uncertainty around max species
lower_slaMax<- as.numeric(round(HPDI( as.vector(longMeans$traitMean[longMeans$speciesname == "Acer_pensylvanicum"]) , prob=0.90 )[2],1))
upper_slaMax <- as.numeric(round(HPDI( as.vector(longMeans$traitMean[longMeans$speciesname == "Acer_pensylvanicum"]) , prob=0.90 )[1],1))

slaMin <- as.numeric(round(min(speciesMeans$traitMean))); speciesMeans
#Uncertainty around max species
lower_slaMin<- as.numeric(round(HPDI( as.vector(longMeans$traitMean[longMeans$speciesname == "Quercus_coccifera"]) , prob=0.90 )[2],1))
upper_slaMin <- as.numeric(round(HPDI( as.vector(longMeans$traitMean[longMeans$speciesname == "Quercus_coccifera"]) , prob=0.90 )[1],1))
...
#Uncertainty around max species
lower_lncMax<- as.numeric(round(HPDI( as.vector(longMeans$traitMean[longMeans$speciesname == "Prunus_persica"]) , prob=0.90 )[2],1))
upper_lncMax <- as.numeric(round(HPDI( as.vector(longMeans$traitMean[longMeans$speciesname == "Prunus_persica"]) , prob=0.90 )[1],1))

lncMin <-  as.numeric(round(min(speciesMeans$traitMean))); speciesMeans
#Uncertainty around max species
lower_lncMin<- as.numeric(round(HPDI( as.vector(longMeans$traitMean[longMeans$speciesname == "Hamamelis_virginiana"]) , prob=0.90 )[2],1))
upper_lncMin <- as.numeric(round(HPDI( as.vector(longMeans$traitMean[longMeans$speciesname == "Hamamelis_virginiana"]) , prob=0.90 )[1],1))
...
#Uncertainty around max species
lower_smMax<- as.numeric(round(HPDI( as.vector(longMeans$traitMean[longMeans$speciesname == "Juglans_cinerea"]) , prob=0.90 )[2],1))
upper_smMax <- as.numeric(round(HPDI( as.vector(longMeans$traitMean[longMeans$speciesname == "Juglans_cinerea"]) , prob=0.90 )[1],1))

smMin <- as.numeric(round(min(speciesMeans$traitMean))); speciesMeans
#Uncertainty around max species
lower_smMin<- as.numeric(round(HPDI( as.vector(longMeans$traitMean[longMeans$speciesname == "Betula_populifolia"]) , prob=0.90 )[2],1))
