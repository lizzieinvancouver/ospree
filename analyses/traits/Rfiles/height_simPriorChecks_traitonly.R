...
mdl.traitonly <- stan("stan/phenology_combined_traitonly.stan",
  ...
  load("output/traitonly_5_5_largeNsppNstudy.Rdata")
...
trt.out  <- data.frame( 
  ...
  pdf("mu_study_esti50sampsize.pdf",width = 10, height = 5)
  par(mfrow = c(1,2))
  ...
  mdl.traitonly <- stan("stan/phenology_combined_traitonly_centeredprior.stan",
                        data = all.data,
                        iter = itterNumber,
                        warmup = warmupNumber,
                        chains = 4,
  ...
  mdl.traitonlyncpcentred <- stan("stan/joint_traitonly_ncp.stan",
                        data = all.data,
                        iter = itterNumber,
                        warmup = warmupNumber,
                        chains = 4,
                        cores = 4,
                        include = FALSE, pars = c("y_hat"))
  save(mdl.traitonlyncp, file="output/traitonly_ncp.Rdata")
  
  #load("output/traitonly_5_5_largeNsppNstudy.Rdata")
  #postMeanSLA <- rstan::extract(mdl.traitonlyncp)
  
  sum.jfcp <- summary(mdl.traitonlyncp)$summary
