...
mdl.jointfcploop <- stan('stan/joint_3cue_newprior_loop.stan',
                     data = pheno_data,  iter = 3000,
                     warmup = 2000,
                     chains = 4)
save(mdl.jointfcploop, file = "output_3cues_loop.Rda")
#load(file = "output/output_compare_smallvslarge.Rda")
####################################################################
# ssm <-  as.shinystan(mdl.traitonly)
# launch_shinystan(ssm)

sum.jfcp <- summary(mdl.jointfcploop)$summary
post.f <- rstan::extract(mdl.jointfcploop)
...
sumer <- summary(mdl.jointfcploop)$summary
pdf("muStudyParaEstiLoop.pdf")
plot(mu.study , sumer[grep("muStudy\\[", rownames(sumer)), "mean"])
abline(a = 0, b = 1, lty = "dotted") # not amazing ... but not horrible
dev.off()

pdf("alphaPhenoSpParaEstiLoop.pdf")
plot(alpha.pheno.sp , (sumer[grep("alphaPhenoSp", rownames(sumer)), "mean"])) #+ sumer[grep("muSp\\[", rownames(sumer)), "mean"]))
