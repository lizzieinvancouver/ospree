...
trt.dat$mu.trtsp <- rep(rep(mu.trtsp, Nstudy), each = Nrep)  #adding ht data for ea. sp
...
for (i in 1:Ntrt){
  trt.dat$mu_grand_sp[i] <-  trt.dat$mu.trtsp[i] +  mu.grand
}

for (i in 1:Ntrt){
  trt.dat$yTraiti[i] <-  trt.dat$mu.trtsp[i] + trt.dat$mu.study[i] +  mu.grand + trt.dat$trt.er[i]
}
...
mdl.traitOld <- stan('stan/trait_only_June27.stan',
                  data = pheno_data, iter = 4000,
                  include = FALSE, pars = c("y_hat"))
sumer <- summary(mdl.traitOld)$summary
