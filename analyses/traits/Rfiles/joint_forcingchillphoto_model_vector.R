...
mdl.jointfcpVect <- stan('stan/joint_3cue_newprior_vector.stan',
                     data = pheno_data,  iter = 3000,
                     warmup = 2000,
                     chains = 4)
save(mdl.jointfcpVect, file = "output_3cues_vector.Rda")
#load(file = "output/output_compare_smallvslarge.Rda")
####################################################################
# ssm <-  as.shinystan(mdl.traitonly)
# launch_shinystan(ssm)

sum.jfcp <- summary(mdl.jointfcpVect)$summary
post.f <- rstan::extract(mdl.jointfcpVect)
...
# stan_hist(mdl.jointfcpVect) # defualt is the firest 10 parameters
# stan_hist(mdl.jointfcpVect, pars = c("muForceSp","muPhenoSp", "sigmaForceSp", "sigmaPhenoSp","sigmapheno_y"))
# 
# 
# post.f2 <- as.matrix(mdl.jointfcpVect, par = c("muPhenoSp", "muForceSp",  "sigmaForceSp","muChillSp", "sigmaChillSp","muPhotoSp", "sigmaPhotoSp", "sigmaPhenoSp","sigmapheno_y"))
# 
