...
# This aim of this script is to summarize the model output of the mean trait + pheno model for leaf nitrogen content specifially, to look at the priors, and to do posterior predictive checks
rm(list=ls())
...
lncData$spps <- lncData$speciesname
...
load("output/phenologyMeanTrait_LNC.RData")
...
# plot(mdl.phen)
pdf("figures/mdl_pheno_lnc_pairs.pdf", width = 10, height = 10)
...
colnames(postMeanTrt_alpaForceSp) <- levels(as.factor(lncData$spps))
...
colnames(postMeanTrt_betaForceSp) <- levels(as.factor(lncData$spps))
...
colnames(postMeanTrt_alphaChillSp) <- levels(as.factor(lncData$spps))
...
colnames(postMeanTrt_betaChillSp) <- levels(as.factor(lncData$spps))
...
colnames(postMeanTrt_alphaPhotoSp) <- levels(as.factor(lncData$spps))
...
colnames(postMeanTrt_betaPhotoSp) <- levels(as.factor(lncData$spps))
...
pdf("figures/MeanLNCphenologyPlots.pdf")
cueEffectPlot
betaTraitxPlot
alphaForcePlot
betaForcePlot
...
alphaPhotoPlot
betaPhotoPlot
write.table(mdl.out,"LNC_summary.txt")
dev.off()
