...
# This aim of this script is to summarize the model output of the mean trait + pheno model for SSD specifially, to look at the priors, and to do posterior predictive checks
rm(list=ls())
...
ssdData$spps <- ssdData$speciesname
...
mean(postMeanTrt$betaTraitxChill) # -0.1342626
...
#muPhenoSp
h1 <- hist(rnorm(1000, 60, 50))
h2 <- hist(postMeanTrt$muPhenoSp)
plot(h2, col=rgb(0,0,1,1/4), xlim = c(0, 300))
plot(h1, col=rgb(1,0,1,1/4), add = T)
...
# plot(mdl.phen)
# pdf("figures/mdl_pheno_ssd_pairs.pdf", width = 10, height = 10)
...
colnames(postMeanTrt_alpaForceSp) <- levels(as.factor(ssdData$spps))
...
colnames(postMeanTrt_betaForceSp) <- levels(as.factor(ssdData$spps))
...
colnames(postMeanTrt_alphaChillSp) <- levels(as.factor(ssdData$spps))
...
colnames(postMeanTrt_betaChillSp) <- levels(as.factor(ssdData$spps))
...
colnames(postMeanTrt_alphaPhotoSp) <- levels(as.factor(ssdData$spps))
...
colnames(postMeanTrt_betaPhotoSp) <- levels(as.factor(ssdData$spps))
...
pdf("figures/MeanSSDphenologyPlots.pdf")
cueEffectPlot
betaTraitxPlot
alphaForcePlot
betaForcePlot
...
alphaPhotoPlot
betaPhotoPlot
write.table(mdl.out,"figures/SSD_summary.txt")
dev.off()
