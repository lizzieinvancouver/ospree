...
# Stem specific density trait only
stemData <- traitsData[traitsData$traitname == "Stem_specific_density",]
...
ospreeData <- subset(ospree, ospree$speciesname %in% unique(stemData$speciesname)) # note change here

# Sorted species and study list
specieslist <- sort(unique(stemData$speciesname))
studylist <- sort(unique(stemData$datasetid))

## Prepare all data for Stan
all.data <- list(yTraiti = stemData$traitvalue,
                 N = nrow(stemData),
                 n_spec = length(specieslist),
                 trait_species = as.numeric(as.factor(stemData$speciesname)),
                 n_study = length(studylist),
                 study = as.numeric(as.factor(stemData$datasetid)),
                 prior_mu_grand_mu = .5,
...
saveRDS(object = mdl.traitphen, file = "SSD_stanfit.RDS")
