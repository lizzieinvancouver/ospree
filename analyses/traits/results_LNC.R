...
if(MidgeFlag == TRUE){
    ...
    posterior <- extract(readRDS(file = "output/LNC_stanfit.RDS"))
}
...
plot.sp <- c("Quercus_ilex", "Populus_tremula", "Alnus_glutinosa") 
col.sp <- c("red", "blue", "orange")

pdf(file = "LNCVisual1.pdf", width = 7, height = 6)
## par(mfrow = c(1, 3))
## Plotting
### Forcing
xrange <- seq(-2.5, 2.5, by = 0.1)
...
axis(side = 2, at = seq(0, 140, by = 20), tcl = -.5, las = 1, cex.axis = 1)
mtext(side = 3, text = "LNC, Forcing", adj = 0, cex = 1.3)
## Add species to plot
...
pdf(file = "LNCVisual2.pdf", width = 7, height = 6)
### Chilling
...
axis(side = 1, at = seq(min(xrange), max(xrange), by = .5), tcl = -.5, cex.axis = 1)
axis(side = 2, at = seq(0, 140, by = 20), tcl = -.5, las = 1, cex.axis = 1)
mtext(side = 3, text = "LNC, Chilling", adj = 0, cex = 1.3)
## Add species to plot
for(i in 1:length(plot.sp)){
    ospree.temp <- subset(ospreeData, ospreeData$speciesname == plot.sp[i])
## Add adjusted column
...
pdf(file = "LNCVisual3.pdf", width = 7, height = 6)
### Photoperiod
xrange <- seq(-1, 2, by = 0.1)
plot(NA, xlim = c(min(xrange), max(xrange)), ylim = c(0, 120),
     ...
     cex.lab = 1.3)
axis(side = 1, at = seq(min(xrange), max(xrange), by = .5), tcl = -.5, cex.axis = 1)
axis(side = 2, at = seq(0, 120, by = 20), tcl = -.5, las = 1, cex.axis = 1)
mtext(side = 3, text = "LNC, Photoperiod", adj = 0, cex = 1.3)
## Add species to plot
for(i in 1:length(plot.sp)){
    ospree.temp <- subset(ospreeData, ospreeData$speciesname == plot.sp[i])
## Add adjusted column
