...
if(MidgeFlag == TRUE){
    ...
    posterior <- extract(readRDS(file = "output/SLA_stanfit.RDS"))
}
...
plot.sp <- c("Quercus_ilex", "Populus_tremula", "Fagus_grandifolia") 
col.sp <- c("red", "blue", "orange")

pdf(file = "SLAVisual1.pdf", width = 7, height = 6)
## par(mfrow = c(1, 3))
## Plotting
### Forcing
xrange <- seq(-2.5, 2, by = 0.1)
...
mtext(side = 3, text = "SLA, Forcing", adj = 0, cex = 1.3)
## Add species to plot
...
pdf(file = "SLAVisual2.pdf", width = 7, height = 6)
### Chilling
...
axis(side = 1, at = seq(min(xrange), max(xrange), by = .5), tcl = -.5, cex.axis = 1)
axis(side = 2, at = seq(0, 140, by = 20), tcl = -.5, las = 1, cex.axis = 1)
mtext(side = 3, text = "SLA, Chilling", adj = 0, cex = 1.3)
## Add species to plot
for(i in 1:length(plot.sp)){
    ospree.temp <- subset(ospreeData, ospreeData$speciesname == plot.sp[i])
## Add adjusted column
...
pdf(file = "SLAVisual3.pdf", width = 7, height = 6)
### Photoperiod
xrange <- seq(-1, 2, by = 0.1)
...
mtext(side = 3, text = "SLA, Photoperiod", adj = 0, cex = 1.3)
## Add species to plot
for(i in 1:length(plot.sp)){
    ospree.temp <- subset(ospreeData, ospreeData$speciesname == plot.sp[i])
## Add adjusted column
