...
for(j in 1:length(study)){
  ...
  maxperc_bb<-max(as.numeric(dat2$response))#maxpercent budburst
  minperc_bb<-min(as.numeric(dat2$response))#minpercent budburst- hopefully zero
