...
npapeallsp_mod<-intxns.all$n[intxns.all$Treatment.1=="total-datasetIDs"]
...
minchill<-round(min(d_modstud$Total_Utah_Model, na.rm=TRUE), digits = 0)
maxchill<-round(max(d_modstud$Total_Utah_Model, na.rm=TRUE), digits = 0)
minforce<-round(min(as.numeric(d_modstud$forcetemp),na.rm=TRUE), digits= 1)
maxforce<-round(max(as.numeric(d_modstud$forcetemp),na.rm=TRUE), digits = 1)
minphot<-min(as.numeric(d_modstud$photoperiod_day),na.rm=TRUE)
maxphot<-max(as.numeric(d_modstud$photoperiod_day),na.rm=TRUE)
