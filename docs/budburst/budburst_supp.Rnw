...
source("..//..//analyses/bb_analysis/revrespcode.bbsupp.R")
...
max.n<-max(as.numeric(d_modstud$n), na.rm=TRUE)
min.n<-min(as.numeric(d_modstud$n), na.rm=TRUE)
@
...
post.standslope.ci<- paste0("(",round((post.standslope[[1]]*7.86/5.68), digits=2), " to ", round((post.standslope[[2]]*7.86/5.68),digits=2), ")")### correct to have both sliding window estimates using the same denominator - see issue #317 for more detailed information
