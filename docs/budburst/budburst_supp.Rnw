...
post.standslope.est <- round((coef(lm(yvar~climate, data=swapost))[2]*swapre_stat$EST)/swapost_stat$EST, digits=2)
post.standslope.ci<- paste0("(",round(((post.standslope[[1]]*swapre_stat$EST)/swapost_stat$EST), digits=2), " to ", round(((post.standslope[[2]]*swapre_stat$EST)/swapost_stat$EST),digits=2), ")")### correct to have both sliding window estimates using the same denominator - see issue #317 for more detailed information
