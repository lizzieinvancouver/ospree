...
nsp<-203#got this from doing: unique(paste(bb.stan$genus,bb.stan$species) with use.flags.for.allsppmodel <- FALSE in models_Stan
nst<-intxns.all$n[intxns.all$treat1=="total-datasetIDstudies"]#

nsp_mod<-67#got this from doing: length(unique(paste(bb.stan$genus,bb.stan$species))) with flags for main bb mod located in snippet_for_ns.R
...
\end{align*}
% Ailene, can you fix the below and ref the relevant tables to first sentence? Also, add Sexpr for my SPECIES numbers (uh, I just guessed)
We applied this model to both a dataset with 203 species/species complexes as well as with 38 species/species complexes. We present estimates from the latter model in the main text (and for FIGS XXX) as it represents species that were more well represented across multiple papers and study designs, and thus are likely more accurately estimated (more details above in section describing the OSPREE database). Based on our modeling approach, species from fewer studies will be pooled towards the overall mean. 
...
% Ailene! Thank you, check my edits below and see what you think ... ref for uneven seasonal warming could be: https://www.eea.europa.eu/data-and-maps/indicators/global-and-european-temperature-9/assessment, or there is another citation (at least) I have, I just cannot find it. Also, we should cite our figures here more. 
% Ailene, still a few details missing ... how do you pick photoperiod for the model in main text Figure 2 (no species) ... see XXX below on where to add. 
Our results integrate over a large range of chilling, forcing and photoperiod conditions (e.g., forcing treatment temperatures ranged from 0-32 \degree C and chilling temperatures ranged from -10-16 \degree C in experiments, as defined by the each study's authors). We also wished to understand how our findings may apply to conditions more commonly found in nature, where conditions often vary dramatically from those applied in controlled environment experiments. For example, very low amounts of chilling can be applied in experiments compared to the natural chilling found in many temperate areas. Additionally, chilling temperature and total chilling are more correlated in nature than in experimental conditions \citep{fig:chillexpfield}. Further, given the importance of chilling and forcing combined with the fact that seasons do not always warm uniformly with climate change (CITES), we also wished to understand how warming in the winter and/or the spring would shift budburst timing. Given these goals we focused on applying our model estimates to defined levels of warming layered onto historical climate. Alternative approaches, such as using climate projections from global circulation models, would have hindered our efforts to understand degrees of warming in different seasons. Further, we emphasize that our predictions are not designed to be accurate forecasts of future budburst dates, even for the locations for which we use historical climate and budburst data. They are designed, however, to provide insights into how natural conditions can differ from experimental conditions, and to provide guidance on how much varying effects of winter and spring warming together will shape future budburst timing.\\
% Our goal was to understand the implications of our model estimates on budburst across varying degrees of spring and winter warming, given natural daily and seasonal cycles in the climate of a well-studied area. 

\par We thus applied our model to Central Europe, a well-studied area for phenology, which thus has both relatively (i.e.,since the 1950s) long-term daily temperature data and budburst data. We selected sites that are part of the Pan European Phenology Project (\url{http://www.pep725.eu}, PEP 725) and included data for two common European species that are prevalent in the OSPREE database: \emph{Betula pendula} (silver birch) and \emph{Fagus sylvatica}. We used a European-wide gridded climate dataset \citep{cornes2018} to extract daily minimum and maximum temperature for the grid cells where observations for these two species were available. We extracted temperature data from 1951 through 1961 (selected as a pre-warming time period) and used these data to estimate annual values for total winter chilling (from 1 September through 30 April, in Utah units, using the R package chillR, see details above in \emph{Estimating chilling} section) and mean spring forcing estimated as the mean temperature from 1 March through 30 April. We inputted these estimates for chilling and forcing into our main model to estimate budburst under `pre-warming conditions,' and then investigated model predictions of budburst given different levels of warming (from 1-7 \degree C) above this baseline, including a full matrix of altered total chilling and forcing estimates (Fig. \ref{fig:apc}). For the results shown in Fig. \ref{fig:apc} (main text) and Fig. \ref{fig:fagsyllat} we set photoperiod [as XXX based XXX reasoning.] We further estimated potential declines in photoperiod due to advancing phenology by first estimating the shift in days to budburst as described above for Fig. \ref{fig:apc}. We then used this budburst date estimate to identify the change in photoperiod between the day of year during the pre- and post-warming periods and then re-fit the model with this new photoperiod. 

\par We applied our model at all latitudes and longitudes included in the PEP database between 1951 and 1961 for \emph{Betula pendula}. We selected two of these sites for \emph{Betula pendula}, as well as two sites where \emph{Fagus sylvatica} occurs, to compare budburst responses across species that differ in their responses to chilling, forcing, and photoperiod, as well as sites that differ in baseline climate (Figures 3, \ref{fig:foremap}).
...
\par Cat - please add a paragraph here describing more about the comparison of your ... % Ailene, I wonder if whatever you want from Cat is covered in the PEP sims section?
...
sp.table <- xtable(sp.st.table, caption="\\textbf{Species included in the OSPREE database}.
", 
...
print(sp.table, include.rownames=FALSE, hline.after = c(-1,0),caption.placement="top",size="\\footnotesize")
@
...
weintable <- xtable(wein.table, caption="\\textbf{Estimates from Weinberger model fit with standardized predictors}. Using a model with Utah chilling units and testing the effects of the Weinberger method and the interaction between this method and the three main environmental cues  show that budburst is generally later for Weinberger studies and the effect of chilling is muted while the effect of forcing is stronger. We present posterior means, as well as 25th, 75th, 2.5th and 97.5th percentiles from models in which the predictors have been standardized so that they are directly comparable.",
...
\item A table that goes with Figure 3 (the 4-paneled 3D forecasting figure) that includes the mean, min, max budburst day of year, chilling estimates, mean winter temperature, and spring (forcing) temperatures for the 4 sites with no warming and with the 7 levels of warming.
