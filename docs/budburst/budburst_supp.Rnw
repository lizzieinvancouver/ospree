...
\par We also tested whether the observed declines in sensitivity could be explained via the absolute sliding time window (SWA) approach \citep{simmonds2019}. CAT: please add some description here.
\par We found that this approach yielded consistent results with our previous analyses: higher sensitivity during the pre-warming time period (Table XS).
