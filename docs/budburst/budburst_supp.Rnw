...
mnSD<-round(mean(d_modstud2$resp_error[d_modstud2$error.type=="SD"]/d_modstud2$response.time[d_modstud2$error.type=="SD"], na.rm=TRUE)*100, digits = 2)
...
\par Our model does not include measurement error because these data were not possible to include for 25 out of 39 experiments included in the OSPREE model with well-represented species. For those studies that did report measurement error, the error was generally small relative to the magnitude of the responses (e.g., standard deviation was, on average \Sexpr{mnSD} \% of the  response variable for studies for which standard deviation was extracted). Thus, it is unlikely that adding measurement error to our analyses would have a large effect on our estimates \emph{\citep{rstan2019}}. \\
