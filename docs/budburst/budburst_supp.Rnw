...
source("..//..//analyses/bb_analysis/get_n.R")
...
ns<-as.numeric(d_modstud$n[-which(is.na(as.numeric(d_modstud$n)))])
max.n<-quantile(ns,probs=0.75)
min.n<-quantile(ns,probs=0.10)
@
...
sm.table <- xtable(smtab, caption="\\textbf{Estimates from model fit with standardized predictors, including only those experiments that included atleast two interactions between cues}. We wanted to understand the effect of removing studies that did not estimate interactions between cues, because such studies might result in , the model using Utah units for chilling. This model was fit to the dataset including species that are well-represented in the OSPREE database, and consisted of 36 species or species complexes across 19 experiments from 16 papers. We present posterior means, as well as 50 percent and 95 percent uncertainty intervals from a model in which the predictors have been standardized so that they are directly comparable.", 
