...
\begin{align*}
y_i \thicksim N(\alpha_{sp[i]} + \beta_{forcing_{sp[i]}} + \beta_{photoperiod_{sp[i]}} + \beta_{chilling_{sp[i]}}, \sigma_y)
\end{align*}

\noindent The $\alpha$ and each of the three $\beta$ coefficients were modeled at the species level, as follows:
\begin{align*}
\alpha_{sp} & \thicksim N(\mu_{\alpha}, \sigma_{\alpha}) \\
\beta_{forcing_{sp}} & \thicksim N(\mu_{forcing}, \sigma_{forcing}) \\
\beta_{photoperiod_{sp}} & \thicksim N(\mu_{photoperiod}, \sigma_{photoperiod})\\
\beta_{chilling{sp}} & \thicksim N(\mu_{chilling}, \sigma_{chilling})
\end{align*}

\noindent We choose weakly informative priors; iincreasing the priors three-fold did not change the model results. 

We ran four chains simultaneously, with 4 000 warm-up iterations followed by X XXX sampling iterations, yeilding XX XXX posterior samples for each parameter. We assessed model performance through $\hat{R}$ close to 1 and high $n_{eff}$ (XX XXX for most parameters, but as low as XXXX for several parameters) as well as visual consideration of chain convergence and posteriors \citep{BDA}. 

In our figures we show means $\pm$ XX\% credible intervals from this model, because of our focus here is on the most likely value for each parameter (e.g., estimated response to forcing) and because they are computationally stable \citep{BDA,Carpenter:2016aa}. See tables XX other XX\% credible intervals. 
...
library(dplyr)
library(rstan)
source("../../analyses/bb_analysis/source/mod_table.R")
mod.table <- xtable(zmodtable, caption="\\textbf{Estimates from model fit with standardized predictors}. The main model we present in the main text uses Utah units for chilling. Using a Chill Portions instead changes the species-level and overal estimates quantitivatively, though the results are qualitatively similar. We also present coefficients from a model included all species, including crops, and all treatment types. We present estimates and their 95 percent credible intervals from models in which the predictors have been standardized so that they are directly comparable,", 
...
print(mod.table, include.rownames=FALSE, hline.after = c(-1,0,9),caption.placement="top",size="\\footnotesize")

nonzmod.table <- xtable(nonzmodtable, caption="\\textbf{Estimates from model fit with predictors on their natural scales}  so that effect sizes can be more readily intepreted in a meaningul way (e.g., change in days of budburst per degree C of warming for forcing temperature). The main model we present in the main text uses Utah units for chilling. We also present coeficients from a model included all species, including crops, and all treatment types. We present estimates and their 95 percent credible intervals from models in which the predictors have been standardized so that they are directly comparable,", 
        label="tab:methods",align=c(
               "p{0.01\\textwidth}|",
                "|p{0.05\\textwidth}|",
               "p{0.15\\textwidth}|",
