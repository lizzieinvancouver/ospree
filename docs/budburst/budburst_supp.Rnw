...
colnames(sp.st.table)<-c("Species","Number of papers","DatasetID")
...
sp.table <- xtable(sp.st.table, caption="\\textbf{Species included in the OSPREE database}", 
        label="tab:sp",align=c(
               ...
               "p{0.1\\textwidth}|",
               "p{0.3\\textwidth}|"))
print(sp.table, include.rownames=FALSE, hline.after = c(-1,0,203),caption.placement="top",size="\\footnotesize",tabular.environment = "longtable", floating = FALSE,align=c(rep('|', ncol(sp.table)),'p{4in}'),width="\\textwidth")
@
