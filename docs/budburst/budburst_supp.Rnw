...
swapre <- read.csv("../../analyses/bb_analysis/pep_sims/simmonds_slidingwin/output/results_swapre_mayref.csv", header=TRUE)
swapost <- read.csv("../../analyses/bb_analysis/pep_sims/simmonds_slidingwin/output/results_swapost_mayref.csv", header=TRUE)

pre.standslope <- as.numeric(confint(lm(yvar~climate, data=swapre))[2,])
pre.standslope <- paste(pre.standslope[[1]], pre.standslope[[2]], sep=" to ")
post.standslope <- as.numeric(confint(lm(yvar~climate, data=swapost))[2,])
post.standslope <- c(((post.standslope[[1]]*swapre_stat$EST)/swapost_stat$EST), ((post.standslope[[2]]*swapre_stat$EST)/swapost_stat$EST)) ### correct to have both sliding window estimates using the same denominator - see issue #317 for more detailed information
post.standslope <- paste(post.standslope[[1]], post.standslope[[2]], sep=" to ")
...
swa$Stand.Slope<-c(pre.standslope, post.standslope)
...
swaresults <- xtable(swa, caption="\\textbf{Absolute sliding time window results} for before climate change (1950-1960) and after climate change (2000-2010). `Temp' is the average temperature, and 'Standardized Slope' is the slope, adjusted to pre-climate change thermal unit days.",
