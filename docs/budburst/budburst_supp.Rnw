...
Previous research has found differences in spring phenology across life stages \citep[e.g., juvenile versus adult trees][]{vitasse2013ont}. We tested for differences in days to budburst across life stages. For this, we included only species or species complexes used in experiments on cuttings from adult trees and on juvenile life stages (seedlings or saplings); this resulted in nine species complexes. The life-stage model was thus:
\begin{align*}
y_i &= \alpha_{sp[i]} + \beta_{forcing} + \beta_{photoperiod} + \beta_{chilling} + \beta_{stage} + \epsilon_{i},
\end{align*}
\begin{align*}
\epsilon_i & \sim N(0,\sigma^2_y) \\
\end{align*}
...
\par We found that this approach yielded consistent results with our previous analyses. This appraoch estimated higher sensitivity during the pre-warming time period (Table \ref{tab:clim}), though when adjusted for growing degrees there was again little difference across the two time-periods (XX$\pm$2.6 in 1950-1960 versus XX$\pm$2.0 in 2000-2010).In addition, during the post-warming time period, the window was open for 18 fewer days and exhibited lower variance than in the pre-warming time period. 
...
cont.table[which(rownames(cont.table)=="$N_{sp}$"),]<-c("36","","","","")
...
stage.table[which(rownames(stage.table)=="$N_{sp}$"),]<-c("36","","","","")
