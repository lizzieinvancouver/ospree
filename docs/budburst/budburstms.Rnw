...
We reviewed 201 papers from controlled environment studies, then extracted data from all experiments that reported budburst responses, yielding data from \Sexpr{nst} studies across 39 years and \Sexpr{nsp} species (Fig. \ref{fig:map}, Tables \ref{tab:ref}, \ref{tab:sp}). %(Table S1, Fig. S1) 
...
#this is wrong! this should be for Fig 3 not Fig 2!
#spests<-read.csv("..//..//analyses/output/betpen_for3dplot/betpen.forecast.forheatmap.csv", header=TRUE)
#chillminbb<-round(240*spests$chill.forecast[which(spests$bb.sprtemp.7==min(spests[,6:13]))], digits=1)
#chillTminbb<-round(spests$winT.forecast[which(spests$bb.sprtemp.7==min(spests[,6:13]))], digits = 1)
#forceminbb<-round(spests$sprT.forecast[which(spests$bb.sprtemp.7==min(spests[,6:13]))], digits = 1)
