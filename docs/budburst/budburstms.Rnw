...
\title{Chilling outweighs photoperiod and forcing cues in temperate trees in experiments, but not in natural systems} % perspective paper for OSPREE analyses
% or Chilling dominates tree budburst in controlled climate experiments, but not in the great outdoors

\author{A.K. Ettinger, C. Chamberlain, D. Buonaiuto, I. Morales-Castilla, E. Wolkovich}
...
\par A simple interpretation of our model---especially its chilling and photoperiod effects---predicts what has been observed in Europe recently: declining sensitivities to warming in long-term data with climate change. This is because even though forcing increases, chilling is expected to decrease and photoperiods will often shorten as budburst advances (cite our photoperiod paper? or something else...)---both predicting delays, and thus an overall muted effect of global warming on spring budburst (Fig. \ref{fig:fore}). Yet such predictions require extrapolating from experimental conditions, which often vary greatly (e.g., ranging from 8 to 16 hours in one experiment, cite figure), to conditions in nature. Understanding the implications of our findings thus requires interpreting them in the context of observed environmental conditions. 
