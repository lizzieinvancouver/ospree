...
\usepackage{setspace}
%\externaldocument[sup-]{..//..//docs/budburst/budburst_supp.tex}
...
npape_mod<-intxns.all$n[intxns.all$treat1=="total-datasetIDs"]
...
totexpchill<-intxns.all$n[intxns.all$treat1=="total-exp-chill"]
weinchill<-intxns.all$n[intxns.all$treat1=="total-fielddate-chill"]
@
...
\citep{koerner2010a}. % Our model also predicts that increased forcing advances budburst, whereas reduced chilling and shorter photoperiods both delay budburst (Fig. \ref{fig:fore}). 
...
\caption{\textbf{Estimates of budburst across a range of forcing temperatures and estimated chilling} (converted to a representative mean temperature, see \emph{Estimating chilling} in the Supplemental Methods) based on overall estimates of chilling and forcing effects (see Fig 1). Maximum advances in budburst occur at intermediate chilling temperatures (e.g., here at \Sexpr{chillminbb} chill units or a mean winter temperatures of \Sexpr{chillTminbb} \degree C) and higher forcing (here at \Sexpr{forceminbb} \degree C).  We set photoperiod to eight hours, which the most common photoperiod treatment in the dataset used. Note that days to budburst is relative to experimental methods and thus not comparable to day of year in the field, shading represents days to budburst.} % Adjust to maybe emphasize that it is a mix of the cues that determines when the earliest budburst happens *and* that these figures suggest at higher degrees of warming, when chilling declines, advances will slow or reverse.
\label{fig:apc}
\end{figure}
\newpage
