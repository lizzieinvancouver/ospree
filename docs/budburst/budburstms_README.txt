Budburst Manuscript README 
OSPREE
Observed Spring Phenology Responses in Éxperimental Environments

<><><><><><><><><><>
Files used to create figures in the main manuscript
<><><><><><><><><><>

#Figure 1: analyses/bb_analysis/models_stan_plotting_justmuplot.R

#Figure 2: analyses/bb_analysis/models_stan_plotting_mu_ests_chilling.R

#Figure 3: analyses/bb_analysis/forecasting/forecast_changebb.R (also used for matching 2D figure in supp and for chill forecast 4-panel figure in sup)

In the text ....
# Numbers of chilling treatments come from tables prepared in ospree/analyses/limitingcues/countintxns/countintxns.R

<><><><><><><><><><>
Files used to create tables and figures in the supplemental materials
<><><><><><><><><><>


#Table 1S: analyses/bb_analysis/maketables.forsupp/make_sp_table_forsupp.R

#Table 2S: analyses/bb_analysis/maketables.forsupp/mod_table.R

#Table 3S: analyses/bb_analysis/maketables.forsupp/mod_table.R

#Table 4S: analyses/bb_analysis/maketables.forsupp/mod_table_lat.R

#Table 5S: analyses/bb_analysis/weinberger.R


#Figure XS: analyses/bb_analysis/forecasting/forecast_changebb_lat.R
#Figure XS: analyses/bb_analysis/weinberger.R

In the text ....
# Numbers of chilling treatments, numbers of studies that tested interactions between cues and thermoperiodicity numbers come from tables prepared in ospree/analyses/limitingcues/countintxns/countintxns.R

# What is budburst definitions and numbers come from ospree/analyses/input/whatisbudburst.csv and ospree/data/whatis_budburst.xlsx

Map of study sites was built by Nacho, using output/bbmapdata.csv which is built in analyses/bb_analysis/studydesignplotsbb.R (see 'Map of sites')
