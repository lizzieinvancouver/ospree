...
\emph{Line 115. The fact that the credible intervals overlap could simply arise if parameters are poorly estimated. A better measure would be the \% difference of the effect sizes. In table S5 the two chilling methods give very similar estimates, but the model including all species gives estimates that are more strikingly different.}

\emph{Congratulations on a fascinating and very detailed study.}
...
We also applied within-group centering to our data and found virtually no effect on our estimates of chilling, forcing, and photoperiod (see table below and compare to Table S6 (Utah units with 36 species) in the Supplemental Materials). As expected, if our data do not have the bias the reviewer is concerned about, our slope estimates were unchanged, while our intercept estimates ($\mu_{\alpha}$ and $\sigma_{\alpha)$) converged on those of our standardized predictor (z-scored) model (in Table S5, Utah units with 36 species). 
...
% AIlene, check edits here. I thought this could read stronger.
...
Table comparing model estimates from main model (e.g., Figure 2 in the main text, Figure S4 and Table S5 in the Supplemental Materials) to estimates from a model that includes a covariance matrix for species-level intercepts and slopes (i.e., random effects), with 95\% uncertainty intervals in parentheses:
...
row.names(varcortab)<-c("$\\alpha_{sp}}$","$\\beta_{forcing_{sp}}$","$\\beta_{photoperiod_{sp}}$","$\\beta_{chilling_{sp}}$") 

colnames(varcortab)<-c("$\\alpha_{sp}}$","$\\beta_{forcing_{sp}}$","$\\beta_{photoperiod_{sp}}$", "$\\beta_{chilling_{sp}}$") 
