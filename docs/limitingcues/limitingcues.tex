...
%% IMC - perhaps we could specify that non-linearities lead to non-stationary relationships between cues and phenology and that we would be concerned with where thresholds (or inflection points of the curves) are. 
...
%% IMC - I don't find it too long, going through this awesome outline, I realize how many points we are touching.
...
\item Outline some studies showing how predictable they are \citep{Schwartz:1997nn,Ellwood2012} \todo[inline]{What other citations/examples can we add?}
...
\begin{enumerate}
\item \citet{yu2010,fu2015} \todo[inline]{Suggest other citations please!} % AKE: This isn't a "failed prediction paper," but a review stating that things are more complex than previously thought and that "considerable uncertainty remains": Camille Parmesan Mick E. Hanley. 2015. Plants and climate change: complexities and surprises. Annals of Botany.
...
\item If you don't work within the range or projected cue range limits of a species, then consider working on informative extremes... %Isabelle: but extremes are extremelly difficult to reproduce in controlled env.
\item Thus you may consider thermal tolerances/limits (and where is the species optimum?) ...These tests could be especially useful for understanding range limits: look at treatments beyond the variation seen within a species' range and see if there is abrupt change or you see continuous change, if no abrupt change then it may suggest that something else must limit range (e.g., biotic cues, minima temperature after which species) \todo[inline]{I like this but worry it feels too far afield. Can we rein it back in or is it okay or should we slim it down?}
%IMC - It is true that perhaps it is far afield. If we decide to leave it in, I would consider emphasizing the fact that while thermal physiology is receiving increased attention to delineate species ranges, for trees that sort of information may not be particularly insightful given the following reasons: (a) there's still little data on CTmax and CTmin for a majority of tree species, (b) there are not huge differences in those thermal tolerances among tree species, and (c) a given tree species may tolerate very high or low temperatures before it dies, but if its phenology advances (or delays) too much, it may cease to reproduce. Thus, we could make the point that thermal tolerances may impose an ultimate filter to discard survival of a species under too cold-warm conditions, but an earlier, perhaps more informative filter is provided by phenology.
% Isabelle thinks we can cut the above section, but I like Nacho's points ....
% \item After you carefully select the cues to study, make your reasoning clear % AKE, IMC, IC all said to cut
\end{enumerate}
...
\item \todo[inline]{Need more here! What will doing what we suggest change about the world?}
%% IMC - Perhaps adding: While understanding, modelling and predicting interactions among cues and their effects on phenology is challenging (we are experiencing that in our BB_models), any advances on this will allow making more accurate predictions, which has valuable implications to more realistically assess the effects of climate change on plant biodiversity, including agricultural and forest species. So there's a clear applied interest in tackling these issues. I know this is too obvious.  
\end{enumerate}
\end{enumerate}
