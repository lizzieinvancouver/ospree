...
\item Can we show an example of a double-cue non-linearity (Flynn data?) ... \todo{So, the models we have are all linear ... but, the Flynn work suggests that chilling levels off at some point so we could do a simple hinge plot maybe? Linear effect of chill + force until chill met, then just force? Or better to do bucket model -- that would be linear effect of chill (endo dormancy) then linear effect of force (eco) ... could also show force alone, versus force+photo?}
...
\begin{itemize} % Cat didn't answer queries beyond first two (conceptual and correlated cues)
