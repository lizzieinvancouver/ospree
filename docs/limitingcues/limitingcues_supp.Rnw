...
\end{enumerate}
We extracted data (using ImageJ for figures, transcribing values from tables and extracting date, location and other methods from the text) from papers on woody plants that test for photoperiod and/or temperature effects on budburst, leafout, or flowering. \citet{ettinger2020} used a subset of these data (studies on budburst for which we could estimate forcing, chilling and photoperiod treatments), while here we present the full database, capturing data from \Sexpr{allpapers} papers (see Table \ref{tab:ref} for a list), of which \Sexpr{numcroppapers} are focused on crops (\emph{Actinidia deliciosa, Malus domestica, Vitis vinifera, Ribes nigrum, Vaccinium ashei, Vaccinium corymbosum, Prunus persica}).  Papers often reported more than one experiment, which we refer to as a `study.' OSPREE includes \Sexpr{allstudies} studies (with the earliest study was conducted in \Sexpr{minyear} \citep{Lamb:1948aa}), which spanned a variety of plant materials, though studies on `seedlings' (\Sexpr{numseedlings} studies) and `cuttings' (\Sexpr{numcuttings} studies) were most common.
...
\caption{Prevalence of number of cues (chilling, forcing, photoperiod) manipulated in studies over time over time.}
  \label{fig:ts}
\end{figure}
...
\caption{Experimental treatments correlate with latitude. Here we show the average latitude of a study (averaged over all latitudes from which tissue was taken) versus the minimum (upper left) and maximum (upper right) forcing treatments and the maximum (lower left) photoperiod treatment and range of photoperiod treatments (lower right, range calculated as maximum versus minimum treatments in a study). Colors represent unique datasets (see Table \ref{tab:ref}).}
  \label{fig:lat}
\end{figure}
% studydesignplots.R
