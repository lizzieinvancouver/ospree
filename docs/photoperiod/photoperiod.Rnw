...
% EMW: I have a couple concerns with this paragraph that I am not sure how to address: (1) It feels rather out of place (what exact point are we trying to make and is this the place to make it?) (2) we introduce threshold above but it feels a little unnecessarily complicated here. 
...
\end{enumerate} %EMW: Below is good!
...
% EMW: Moved stuff to caption -- it really broke up the flow -- I think the updated caption still needs work; please check. 
...
Indeed, growth chamber studies demonstrate that the magnitude of daylength shifts we can expect with climate change (i.e., 1-2 hours of difference in daylength with temporal shifts over the next century) are substantial enough to affect spring phenology in trees (Table S1). The direction and magnitude of responses will vary, however, because of variation in photoperiod sensitivity, and because photoperiod often interacts with other environmental drivers, such as temperature, to affect phenology (Box 1). % EMW: Better to cite zydlewski2014 in the box (the current formation implies our whole box info overlaps with zydlewski2014
...
% EMW: I have a few issues with the following three paragraphs that I am not sure how to sort out ..
% (1) Threshold is treated odd again -- we start a paragraph with it out of nowhere and then later in the paragraph the language feels like we didn't just mention thresholds (' because photoperiod sensitivity is thought to be a threshold response').
...
% (3) We semi-repeat ourselves: in the first paragraph we say 'This may in turn affect performance under climate change, since the ability to advance phenology with warming has been linked with increased growth, abundance, and fitness \citep{muir1994,cleland2012,willis2010}.' and then in the last (third below) we again say 'Indeed, phenological tracking of temperature (i.e., earlier flowering or leafout with warming) has been linked with higher performance in plants \citep{cleland2012}.' Again, I wonder if something bad has happened to the flow via changes to the below paragraph .... 
...
% EMW: Should we just spell out and explain a G x E interaction here? It might help ... for example an altered version of the first sentence could read (and then adjust the rest of the paragraph from there?):
% \par A challenge in understanding biological responses to shifts in photoperiod is the wide range of sensitivity to photoperiod observed across species \citep{Sanz-Perez:2009aa, zohner2016,flynn2018}, populations \citep{tanino2010}, and ecotypes \citep{Howe:1995aa}, with few studies that specifically quantify how much genotype versus environment explain this variation. 
...
\section*{Conclusions}
% EMW: Can you change 'Here we have addressed how an altered photoperiod may affect woody plant budburst (Table S1, Figure \ref{fig:photomap}).' to be more about what you HAVE shown and outlined in the paper -- bring together what readers should have learned (which is hopefully more than that 'altered photoperiod may affect woody plant budburst' as we all knew that before). 
...
\section* {Glossary}
\begin{itemize}
\item \underline{budburst}: the emergence of new leaves on a plant at the beginning of the growing season. %EMW: Maybe ask Cat or someone to fix this? I don't think budburst is only at the start of the season and we may need to be more explicit in the 'emergence of new leaves' part .... 
...
\item \underline{threshold photoperiod}: length of day that causes an individual to switch from a long-- to a short--day response (or vice versa). %EMW: Can you give an example? This seems critical for readers to understand I think. 
\end{itemize}
...
%EMW - add citation as mentioned below AND I wonder if we should not include budset studies? You mention them below but the rest of the paper is about spring so it feels off. 
