...
\par Here, we ask: 
\begin{enumerate}
\item How will climate change alter the experienced photoperiod by plants? 
\item What are the implications of altered experienced photoperiods for biological responses to climate change?
...
\par Species experience different photoperiod regimes depending on their location on Earth, the seasonal timing of their activity, and inter-annual variation in climate (Fig. \ref{fig:spacetime}-\ref{fig:greenup}). Consider, as an example, the daylength experienced by plants on the date that spring ``green-up" occurs. Spring green-up varies with latitude (Fig. \ref{fig:greenup}a, we use green-up date as an example  because it represents an important plant spring event, signaling the start of the growing season, with globally-available estimates), in part because latitudinal variation in green-up date, which occurs earlier toward the equator and later toward the poles, is strongly driven by climatic differences that affect phenology, and in part because of latitudinal variation in photoperiod (e.g., at the poles, the daylength at the summer solstice is 24 hours; see also Fig. \ref{fig:spacetime}). 
