...
ospree<-read.csv("../../analyses/output/ospree_clean_respvar.csv",header=T)#which version should I use?
...
#photop<-photop[-which(photop$photoperiod_day=="half in 16, half ambient(9.5-10.5)"),]#remove this one for now-too wide for the table
...
photop_max<-aggregate(as.numeric(photop$photoperiod_day), by=list(photop$datasetID,photop$continent,photop$provenance.lat,photop$provenance.long), FUN=max)
colnames(photop_max)[5]<-"daylength_max"
colnames(photop_min)[5]<-"daylength_min"
photop_all<-join(photop_min,photop_max, by=c("Group.1", "Group.2", "Group.3","Group.4"), match="all")
colnames(photop_all)[1:4]<-c("study","continent","lat","long")
...
photop_table<-subset(photop_all, select=c(study, continent,lat, long,daylength,space,time))

#,"daylength","spatial","temporal"

photop.table<-xtable(photop_table,digits=3,
  caption="Growth chamber experiments and their photoperiod treatments.",label="table:phototreats",align=c(
                "|p{0.01\\textwidth}",            
                "|p{0.15\\textwidth}",
                "|p{0.15\\textwidth}",
                "|p{0.08\\textwidth}",
