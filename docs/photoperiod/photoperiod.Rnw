...
\begin{enumerate}
\item Photoperiod is a critical cue used by organisms to synchronize their activities with seasonal climatic changes (need other citations- there are many possibilities!)\citep{hsu2011,singh2017}.
...
photop <- ospree %>% # start with the data frame
  ...
  dplyr::select(datasetID, provenance.lat, provenance.long, photoperiod_day)#growing or provenance lat/long?
...
colnames(photop)<-c("study","lat","long","daylength","delta","spatial","temporal")

photop.table<-xtable(photop,digits=3,
