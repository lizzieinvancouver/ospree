...
\item Temporal shifts yield bigger changes in photoperiod than spatial shifts  (Figures \ref{fig:photo},\ref{fig:photo_bar}). 
\item Shifts in photoperiod may vary with latitude (Figure \ref{fig:photo_bar}). 
\item Vartiation in phenology also varies with latitude (cite a pretty map showing variation in spring index or greenup?), so it is unclear how these two things interact....
\end{enumerate}
\item\textit{What are the implications of these altered photoperiods for forecasts of climate change impacts?}
\begin{enumerate}
\item Phenology will be affected, given that daylength is known to affect cell elongation and budburst \citep{linkosalo2006,erwin1998,sidaway2010}.
...
\item Most experiments manipulate photoperiod much more dramatically than will occur with climate change (but see \citep{basler2012}), so it is difficult to extrapolate findings.
\item Magnitude of effects of photoperiod may be small, in comparison to warming effects, but we don't really know...
\end{enumerate}
...
\section* {To do if we like this format:}
\begin{enumerate}
\item Make a map showing annual variation in global greenup (to make the point that it greenup up is more stable at some latitudes than others?)
\item Make Table of studies and treatments
\item Focus on phenology only? (or use it as a case study?)
\item What other points do we want to make about photoperiod?
\end{enumerate}
...
\begin{footnotesize} 
   \begin{tabular}{| p{1cm} | p{3.8cm} | p{2.9 cm} | p{3.2 cm} | p{3.3 cm} |}
    \hline
  \bf{Study} & \bf{Photoperiod treatments (or delta)} & \bf{Spatial equivalent} & \bf{Temporal equivalent} & \bf{Finding (sensitive?)}\\ \hline
    basler12 & 9.5, 11 (1.5) &  600 km up & 30 days earlier & Yes (late successional only) \\ \hline
    laube14a & 8,16 (8) &  3200 km up & 160 days earlier & Yes (some species)\\ \hline
    other studies &  &   &  & \\ \hline
