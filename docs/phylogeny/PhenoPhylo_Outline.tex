...
Traditional phylogenetic regression methods, were originally conceived as a statistical correction for phylogenetic non-independence across observations ---i.e. species---, so that linear regression assumptions were not violated \citep{freckleton2002phylogenetic}. This correction incorporates phylogenetic structure in the regression by modifying the residual Variance-Covariance matrix to substitute off-diagonal elements of zero (under the assumption of independence across observations) for shared phylogenetic branch lengths representing pairwise covariances (under phylogenetic non-independece among observations). Off-diagonals may be allowed to include a multiplying parameter---i.e. the lambda parameter---which is a transformation indicating the amount of phylogenetic relatedness among species (see below). Because the aim was to correct for statistical nuance, the underlying assumption of phylogenetic regressions is that phylogenetic relatedness would only affect either model residuals in PGLS approaches \citep{freckleton2002phylogenetic} where model intercept does not vary across species, or to species intercepts in PMM approaches \citep{housworth2004phylogenetic}.\\ 
...
Because our aim is to understand how evolution may have imprinted biological responses to multiple interactive cues, our approach expands the above methods by explicitly incorporating phylogenetic structure to model intercepts, slopes and residuals so that it is possible to estimate  relatedness in the sensitivities to each cue, when considered together.
...
\begin{comment}
% this is a legacy from an old version of the ms. I believe we agreed not to follow this path (of forecasting our models under Climate change). This bit will be removed in the next revision, just leaving it here for now in case someone wants to comment or have suggestions.
