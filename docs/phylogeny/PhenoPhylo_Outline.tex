...
The above patterns were strongly phylogenetically clustered for forcing ($\lambda$ \= 0.68), moderately so for chilling ($\lambda$ \= 0.56) and weakly for photoperiod ($\lambda$ \= 0.24) (see Fig. \ref{fig:muplot_force}, Table \ref{tab:modelanglamb}). This indicates that species responses to interacting cues, when modelled jointly, widely differ in how they are affected by evolutionary relatedness. Sensitivity to photoperiod treatments did not vary across clades while responses to forcing tend to be more similar among closely related species (Fig. \ref{fig:muplot_all}).   
...
%% from methods
% IMC 15mar perhaps we do not need the below bit, but I thought it could be helpful to show how our model specification compares to that of more classic approaches. I have to double check the equations below, and introduce the indices for partial pooling on slopes and intercept (not sure it works as is, please Lizzie, Deirdre, Geoff, can you check this?)
% EMW (19Mar2022): I chatted with JD about this and I think we should be cautious about too much focus on PGLS versus PMM versus our model here -- if we do this, we're inviting a lot of methods-focused reviewers and I don't see that as a good outcome. So I would recommend that we focus on what our method does BEYOND PGLS or PMM and stop there. (So, cut the below equations.) 
For the sake of comparison, the PGLS version of equation \ref{eqfive} would be written:

\begin{align}
  \label{eqPGLS}
  \mu_j = \mu_\alpha + \mu_{\beta_1} X_1 + \mu_{\beta_2} X_2 + \mu_{\beta_3} X_3 + e_{\alpha_{j}} 
\end{align}
...
\begin{align}
  \label{eqPMM}
  \mu_j = \mu_\alpha + \mu_{\beta_1} X_1 + \mu_{\beta_2} X_2 + \mu_{\beta_3} X_3 + e_{\alpha_{j}} + e_{\epsilon_{j}} 
\end{align}

where e_$\epsilon$ is the error term independent of phylogenetic effects. Thus, the model specification used here \ref{eqfive} can be seen as an extended version of the above, where the error term is decomposed one step further, allowing to partition residual variance into predictor-specific components. 
