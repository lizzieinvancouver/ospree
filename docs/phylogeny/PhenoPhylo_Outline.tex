...
Accounting for the effects of phylogenetic structuring on the effects of jointly modelled cues had an effect on model coefficients both for angiosperms (Fig. \ref{}) and gymnosperms (Fig. \ref{}). Not accounting for phylogeny (or assuming $\lambda \= 0) biased model coefficients, particularly so for forcing and somewhat less for chilling (Fig. \ref{}). Specifically, species sensitivities to forcing and chilling were underestimated on average (by XX and XX days per unit of standardized cue, respectively). Sensitivities to photoperiod, which showed weak phylogenetic signal were unsurprisingly not biased in non-phylogenetic models and model intercepts were not affected either (Fig. \ref{}). Similarly
