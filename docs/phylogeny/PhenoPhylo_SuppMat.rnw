...
\item Iteratively subset the dataset leaving out one genera at a time. For our approach here, we report results from leaving out (iteratively) the 25 genera with at least two species, and (of those genera) the highest number of observations. 
\item Run both PMM and HMM for each subset of data recording the posterior distribution of predicted values. 
\item Compare the observed response of left out (held out) species against the predicted response for those species inferred by the subset version of the models that excluded those species. % EMWOct15: Are you taking the posterior means here? Be specific if possible. 
