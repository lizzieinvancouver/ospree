...
Mitigation and adaptation of human systems to ongoing climate change hinge on ecological forecasting to predict shifts in key ecosystem services, such as carbon storage. Unfortunately, progress towards better predictions of biological responses to climate change is hindered by non-linear relationships, data imbalance and large variability across species. Here we use a Bayesian hierarchical phylogenetic model to estimate species-level phenological responses to major environmental cues---i.e., temperature, daylight. This modelling scheme overcomes data-driven bias %IMC- not sure about including here something like (not accounting for phylogeny pulls estimates towards overrepresented species)
