...
To address different day/night or ramped temperatures we take a weighted hourly average over the full period of forcing. We also adjusted for differences caused by co-varying thermo- and photo-periodicity for both the forcing temperatures and photoperiod values.% IMC - this needs editing.
...
%IMC - I'm not sure we need this last bit, but I realize we do not explain the HMM model, and otherwise I wouldn't know where to fit the study as a random effect issuem
