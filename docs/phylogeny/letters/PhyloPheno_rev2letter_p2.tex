...
We would agree with the reviewer that our paper does both. We present a novel statistical approach applied to an important empirical question (and related dataset) and find species-level variation that reshapes how we understand phenological cues, then show how this impacts forecasting. We mention this in detail (\lr{whatisyourpaper3}-\lr{whatisyourpaper3end}) but have now made it more clear through small changes to the abstract (\lr{whatisyourpaper1} and \lr{whatisyourpaper2}) discussion (e.g., \lr{whatisyourpaper4}). 
