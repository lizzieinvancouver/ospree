...
%DIF DEL revision1/PhenoPhylo_ms_rev1.tex   Tue Oct 31 07:12:06 2023
%DIF ADD PhenoPhylo_ms.tex                  Fri Mar 29 14:14:21 2024
...
%DIF < 
%DIF -------
\newcommand{\R}[1]{\label{}\linelabel{#1}} %DIF > 
\newcommand{\lr}[1]{line~\lineref{#1}} % Not actually sure I needed this %DIF > 
%DIF -------
...
\DIFaddend \section*{Abstract} %DIF >  148 words currently
Our ability to adapt to climate change requires accurate ecological forecasting \DIFdelbegin \DIFdel{to predict }\DIFdelend \DIFaddbegin \DIFadd{of }\DIFaddend shifts in key ecosystem services, such as carbon storage and biodiversity maintenance. Current forecasts, however, have \DIFdelbegin \DIFdel{generally }\DIFdelend failed to capture \DIFdelbegin \DIFdel{the }\DIFdelend important variability in biological responses, especially \DIFdelbegin \DIFdel{observed }\DIFdelend across species. \DIFdelbegin \DIFdel{Here, using novel }\DIFdelend \DIFaddbegin \R{whatisyourpaper1}\DIFadd{Here, we present a novel method using }\DIFaddend Bayesian hierarchical phylogenetic models \DIFdelbegin \DIFdel{, we }\DIFdelend \DIFaddbegin \DIFadd{to }\DIFaddend overcome this challenge \DIFdelbegin \DIFdel{to }\DIFdelend \DIFaddbegin \DIFadd{and }\DIFaddend estimate species-level responses.  
We \DIFdelbegin \DIFdel{illustrate our method with an dataset on phenological responses to }\DIFdelend \DIFaddbegin \DIFadd{apply our method to phenological experiments manipulating }\DIFaddend temperature and daylength cues \DIFdelbegin \DIFdel{, determined experimentally for 191 woody species.
%DIF < IMC - I tried to accommodate reviewer#2 suggestion above describing the data we use, please feel free to edit.
%DIF <  EMW14Oct -- Edited above, as it read funny. See what you think. 
We find that variation is greater across species than across }\DIFdelend \DIFaddbegin \DIFadd{of woody plant species. We show species differences dwarfed the average differences between }\DIFaddend cues, suggesting the current focus on \DIFdelbegin \DIFdel{identifying which cue is the strongest may be less important than predicting how each species responds to a combination of cues}\DIFdelend \DIFaddbegin \DIFadd{comparing across cues may be misdirected}\DIFaddend . Further, our model provides insights on how evolutionary history has shaped responses to cues, \DIFdelbegin \DIFdel{suggesting }\DIFdelend \DIFaddbegin \DIFadd{indicating }\DIFaddend stronger evolutionary dynamics in responses to temperature---especially cool winter temperatures---than daylength. Our \DIFaddbegin \R{whatisyourpaper2}\DIFadd{new }\DIFaddend approach provides a major advance in ecological forecasting, with implications for predicting the impacts of climate change and other anthropogenic forces on \DIFdelbegin \DIFdel{species interactions and ecosystem functioning}\DIFdelend \DIFaddbegin \DIFadd{ecosystems}\DIFaddend .
\clearpage
%DIF >  Alt but still 150 words:
%DIF >  Our ability to adapt to climate change requires accurate ecological forecasting of shifts in key ecosystem services, such as carbon storage and biodiversity maintenance. Current forecasts, however, have failed to capture important variability in biological responses, especially across species. \R{whatisyourpaper1}Here, we present a novel method using Bayesian hierarchical phylogenetic models to overcome this challenge and estimate species-level responses. Combining our new method with phenological experiments manipulating temperature and daylength cues of woody species, we find much higher variation across species than cues, suggesting species-level variation may be more important than the current focus on comparing across cues. Further, our model provides insights on how evolutionary history has shaped responses to cues, suggesting stronger evolutionary dynamics in responses to temperature---especially cool winter temperatures---than daylength. Our \R{whatisyourpaper2}new approach provides a major advance in ecological forecasting, with implications for predicting the impacts of climate change and other anthropogenic forces on ecosystems.
...
% EMW13Oct - I took about 'know' above -- maybe thus confused R3 into thinking we have TRUE values?
% EMW14Oct -- I removed ' from species' as we do not use it consistently. I also tried to consistently add a , after e.g. and i.e. (Americans do, Brits don't; here we just need to be consistent). 
...
%These studies have also found remarkable variation across species in their sensitivities to cues, aligning with long-term observational data across thousands of species \citep{XXX}, and suggesting that current models and forecasts fail to capture the complexity of species-specific responses. 
...
%DIF >  couldcut (Mar2024): could probably cut more of this paragraph
...
Similar clade-level variation was observed for forcing, where some of these clades---e.g., Ericaceae, Rhamnaceae, Ulmaceae, or Fagaceae---were particularly sensitive \DIFdelbegin \DIFdel{(advancing their budburst more than 10 days per standardized unit of forcing) }\DIFdelend and others, such as the Sapindaceae, Cornaceae or Juglandaceae, show little response (Fig. \ref{fig:muplot_all}b). %JD - perhaps could link back to invasiveness? Species more sensitive to forcing more invasive? %IMC11Dec - This is a very nice idea... perhaps we can comment on it later in the discussion.
...
\DIFaddbegin \R{whyuse3}\DIFaddend While we focused on spring phenology here, our \DIFaddbegin \DIFadd{new }\R{whatisyourpaper4}\DIFaddend approach suggests a path forward for more general forecasting of species-level climate change responses. Our results show how including the phylogenetic relationship of species in a mechanistic model of underlying cues can overcome major limitations of most current hierarchical models---correcting biased model estimates, estimating the full variability across species and reducing uncertainty around individual species estimates---while at once providing insight into the evolutionary history of biological responses. Using this approach improved forecasts of phenological responses to climate change and could help anticipate impacts on critical ecosystem services from species-level shifts and thus aid mitigation and human adaption to warming. \DIFaddbegin \R{whyuse3E}\DIFaddend %IMC19may - perhaps we could go beyond laying out the relevance of our findings (emphasize or provide examples of additional potential applications in ecological forecasting?).  
...
\end{enumerate}
We scraped data from all papers of woody species that tested for photoperiod and/or temperature effects on budburst, leafout, or flowering, resulting in data from 155 experiments across 97 papers in the updated database. \citet{ettinger2020}\DIFaddbegin \DIFadd{, which }\DIFaddend used a portion (72 experiments across 49 papers) of the earlier OSPREE database\DIFdelbegin \DIFdel{and }\DIFdelend \DIFaddbegin \DIFadd{, }\DIFaddend provides extensive methods on  \DIFdelbegin \DIFdel{the }\DIFdelend database creation and cleaning.

We focused on angiosperms (as gymnosperms are very poorly represented in spring phenology experiments), and included all budburst experiments where we could quantify chilling, forcing and photoperiod levels, resulting in 44 studies from 33 papers, \DIFdelbegin \DIFdel{resulting in }\DIFdelend \DIFaddbegin \DIFadd{and }\DIFaddend 2940 data points. 
% See phylo_ospreepapercount.R
% We need to publish an updated OSPREE dataset on KNB eventually.
In our dataset most studies come from Europe ($n$=37) and a few from North America ($n$=7). The same bias towards Europe is found across the full OSPREE dataset with less North American ($n$=19) than European ($n$=60) studies and only 3 studies located in the Southern Hemisphere. Given our need of daily gridded data for estimating chilling we only include studies from Europe and North America \DIFdelbegin \DIFdel{, with most of these sites in temperate areas and a few in European Mediterranean areas }\DIFdelend (see Fig. \ref{fig:mapstudylocations}). Our final dataset is both geographically and taxonomically limited, mirroring the existing literature on phenological experiments \DIFdelbegin \DIFdel{. Enlarging this body of literature will be critical to address important research questions, such as whether variability in cue sensitivity across species is larger in temperate than in tropical latitudes. 
%DIF < IMC14OCT - please Lizzie, double check this last bit. EMW -- done!
}\DIFdelend \DIFaddbegin \DIFadd{and highlighting a critical need to expand this literature. 
}\DIFaddend 
