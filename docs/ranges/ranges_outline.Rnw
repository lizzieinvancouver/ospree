...
\begin{enumerate}
\item Phenology is tradeoff between frost risk and growth shape phenological optimums. Trees evole cues to handle this
\item Species have different cues
\item Species have different ranges, ie different frost risk
\item Intuitively these should be related, but not well tested
\item In this paper we a) define frost risk, b) review the predicted reationship between cues and risk and c) test the shit out of this this with epic joint model.
...
Decades of research on phenology have elucidated these environmental cues, suggesting that warming spring temperatures (forcing), cool winter temperatures (chilling) and day length (photoperiod) are primary environmental cues utilized by woody plants to determine the timing of spring phenological events \citep{Ettinger:2020aa,Forrest2010}. Yet These studies also demonstrate the there are substantial cue-use differences among species, with some species relying more heavily on some cues over others \citep{Laube:2014aa,}. 

Decades of other research (biogeohraphy, climatology) inform us that species occupy difference climate spaces, and ecounter 



%As anthropogenic climate change has already driven shifts in spring phenology \citep{Menzel2006}, identifying these inter-specific differences in cue use has emerged as a major goal of phenological research \citep{Chuine2002}. These differences have strong implications for both predicting the rate of phenological shifts as the climate continues to warm \citep{?}, and anticipating the ecological consequences of these shifts \citep{Cleland2012}. 
