...
%\item calculation of GDD last frostnormal(
%\item STV
...
\begin{align*}
y & \thicksim normal(\alpha_{study[i]} + \alpha_{sp[i]} + \alpha_{pop[i]} +& \beta_{forcing_{sp[i]/pop[i]}} + \beta_{photoperiod_{sp[i]/pop[i]}} + \epsilon_[i]} ) \\
\end{align*}
...
\begin{align*}
\alpha_{study} & \sim normal(\mu_{\alpha_study}, \sigma_{\alpha_study}) \\
\alpha_{sp} & \sim normal(\mu_{\alpha_sp}, \sigma_{\alpha_sp}) \\
\alpha_{pop} & \sim normal(\mu_{\alpha_pop}, \sigma_{\alpha_pop}) \\
\beta_{forcing_{sp/pop}} \thicksim normal(\mu_{forcing_[sp/pop]}, \sigma_{forcing_[sp/pop]}) \\
\beta_{photoperiod_{sp/pop}} \thicksim normal(\mu_{photoperiod}, \sigma_{photoperiod_[sp/pop]}) \\
\end{align*}
