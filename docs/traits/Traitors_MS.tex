...
In our model for SLA, we found all three cue responses to be affected by SLA , but the strongest effects from forcing and chilling cues. Including the effect of SLA had a complimentary effect on the species slopes, causing them to become more negative. With the exception of photoperiod responses, this resulted in the slopes of high and low SLA species to become more similar. \\
-neg relationship between SLA and cue responses for forcing and chilling - btraitx = XXX, when we include the trait value in our model we see the slopes become more negative, high SLA sp that are more conservative do have more extreme/negative cue responses than the low sla spp (new sm fig)
- avoid causality, more linking -- affected

1. We found that as SLA increased, responses to each cue generally became more negative.
2. tell reader what the btraitvalue is - negative with 90\% credible interval
3. Describele the panel, including the trait value moves the slopes in x direction
4. The relationship in the new beta vs grand_mean plot
%In line with our our prediction, the high SLA species had earlier budburst dates than the low SLA species.\\

When the betatrait value overlapped with zero - so there is no strong trend

Seed mass - log 10 

Geoff Legault11:40 AM
"We found that as SLA increased, responses to each cue generally became more negative."
For example...at low SLA values...this would happen at high SLA this would happen 

Overall, we found that conservative species, which generally had high traits values for SLA, Height, etc., shifted their phenology more and earlier in response to forcing, chilling, and photoperiod, than acquisitive species with low trait values. This responsiveness is linked to later budbursting - earlier sp have lower slopes bc they require less cues in general, having a lower intercept

- but bc phenolgoy is intercept + slope *Force etc this doesn't mean....

Cue slopes varied from this value to that
\begin{figure}[h!]
    \centering
...
The effects of height on cue responses were complimentary for all three cues, resulting in more negative slopes and advances in budburst. The magnitude of these effects were greatest for forcing and chilling cues. As we predicted, taller trees associated with a more conservative growth strategy had later budburst than shorter more acquisitive species.\\
...
The effect of LNC on cue responses were complimentary, resulting in a more negative slopes for chilling and forcing cues. However, no considerable relationship was observed between LNC and photoperiod. As we predicted, species with high LNC values budburst earlier than species with low LNC. 
...
Our model estimates for the trait effects of seed mass on cue responses were all negative, however, given the nature of the log10 transformation and the resulting negative trait values, the inclusion of trait effects creates positive estimates and offset the effects of the cue responses in the absence of traits. Both acquisitive and conservative species exhibited a greater overlap in their budburst dates for both forcing and photoperiod responses. \\ % acq. vs con spp are responding differently, green = the slope is becoming less negative for F & C and not really changing for P, but for purprle, the slopes are all becoming more negative
