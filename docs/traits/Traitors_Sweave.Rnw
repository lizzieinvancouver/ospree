...
\date{\today}
\author{Deirdre Loughnan$^1$, Faith A M Jones$^1$, Geoffrey Legault$^1$, Mira Garner$^2$, Darwin Sodhi$^3$
, Daniel Buonaiuto$^4$, Catherine Chamberlain$^5$, Ignacio Morales Castilla $^6$, Ailene Ettinger$^7$,  and E M Wolkovich$^1$}
\maketitle 
...
There were a limited number of species in the height model where the simple geometric mean fell outside of predicted species means after accounting for the effect of study, for example \textit{Quercus ilex}, \textit{Quercus petraea}, \textit{Quercus coccifera}, \textit{Aesculus hippocastanum}, and \textit{Rhamnus cathartica}.\\

We estimated mean height as \Sexpr{htMean} m (90\% credible interval: \Sexpr{lower_htMean}, \Sexpr{upper_htMean}). Species height values were distributed with a standard deviation  of \Sexpr{htSD} m (90\% credible interval: \Sexpr{lower_htSD}, \Sexpr{upper_htSD}), and study height values were distributed with a standard deviation of \Sexpr{htStudySD} m (90\% credible interval: \Sexpr{lower_htStudySD}, \Sexpr{upper_htStudySD}). The highest estimated mean species height in our dataset was \Sexpr{htMax} m (90\% credible interval: \Sexpr{upper_htMax}, \Sexpr{lower_htMax}) for \textit{Quercus petraea}, and the lowest estimated mean height value was \Sexpr{htMin} m (90\% credible interval: , \Sexpr{upper_htMin}, \Sexpr{lower_htMin}) for \textit{Rhamnus cathartica}. \\

We estimated mean SLA as \Sexpr{slaMean}  (90\% credible interval: \Sexpr{lower_slaMean}, \Sexpr{upper_slaMean}). Species SLA estimates were distributed with a standard deviation  of \Sexpr{slaSD}  (90\% credible interval: \Sexpr{lower_slaSD}, \Sexpr{upper_slaSD}), and study SLA estimates were distributed with a standard deviation  of  \Sexpr{slaStudySD}  (90\% credible interval: \Sexpr{lower_slaStudySD}, \Sexpr{upper_slaStudySD}). The highest estimated mean species SLA estimate in our dataset was  \Sexpr{slaMax}  (90\% credible interval: \Sexpr{lower_slaMax}, \Sexpr{upper_slaMax}) for \textit{Acer pensylvanicum}, and the lowest estimated mean LNC estimate was  \Sexpr{slaMin}  (90\% credible interval: \Sexpr{lower_slaMin}, \Sexpr{upper_slaMin}) for \textit{Quercus coccifera}.\\

From our model of log$_{10}$ seed mass, the mean was estimated as \Sexpr{smMean}  (90\% credible interval: \Sexpr{lower_smMean}, \Sexpr{upper_smMean}), which translates to a seed mass of approximately 63 mg. Species log$_{10}$ seed mass values were distributed with a standard deviation  of \Sexpr{smSD} (90\% credible interval: \Sexpr{lower_smSD}, \Sexpr{upper_smSD}), and study log$_{10}$ seed mass values were distributed with a standard deviation  of \Sexpr{smStudySD} (90\% credible interval: \Sexpr{lower_smStudySD}, \Sexpr{upper_smStudySD}). The highest estimated mean species log$_{10}$ seed mass estimate in our dataset was \Sexpr{smMax} (90\% credible interval: \Sexpr{lower_smMax}, \Sexpr{upper_smMax}) for \textit{Juglans cinerea}, which translates as a seed mass of approximately 10000mg. The lowest estimated mean log$_{10}$ seed mass estimate was \Sexpr{smMin}  (90\% credible interval: \Sexpr{lower_smMin}, \Sexpr{upper_smMin}) for \textit{Betula populifolia}, which translates as a seed mass of approximately 0.1 mg.\\
...
\begin{figure}[h!]
    \centering
 \includegraphics[width=\textwidth]{..//..//analyses/traits/figures/cuetrait_wtrend_nprior.pdf} 
    \caption{Trait relationships with cue slopes}
    \label{fig:cuetraits}
\end{figure}
The relationship between species height and cue responses was varied across the tree cues, with increasing tall trees having more positive responses to warming, but negative responses to both chilling and photoperiod (Fig. \ref{fig:cuetraits}). Of the three cues, height had the strongest influence on chilling, with an estimated interaction parameter of \Sexpr{htBCSpMean} (90\% credible interval: \Sexpr{lower_htBCSpMean}, \Sexpr{upper_htBCSpMean}). The response to photoperiod was also negative, but the photoperiod slope was smaller at \Sexpr{htBPSpMean} (90\% credible interval: \Sexpr{lower_htBPSpMean}, \Sexpr{upper_htBPSpMean}). Interestingly, forcing had a positive slope, at \Sexpr{htBFSpMean} (90\% credible interval: \Sexpr{lower_htBFSpMean}, \Sexpr{upper_htBFSpMean}). Tall trees like \textit{Acer pseudoplantanus}, have more negative slopes in the full model compared to models with a zero trait effect, and a much greater difference in their slopes than shorter trees like \textit{Corylus avettana} (Fig.\ref{fig:slopes}). The trait effect of photoperiod is much smaller, with the slopes of the full model and model without the effect of trait being similar. 
...
Species log seed mass had a negative relationship with all three cue responses \ref{fig:cuetraits}. As we observed for other trait models, seed mass had the largest influence on chilling  (interaction parameter: \Sexpr{smBCSpMean}: 90\% credible interval: \Sexpr{lower_smBCSpMean}, \Sexpr{upper_smBCSpMean}). The influence of seed mass on forcing was also negative, with a slope of \Sexpr{smBFSpMean} (90\% credible interval: \Sexpr{lower_smBFSpMean}, \Sexpr{upper_smBFSpMean}), as well as its influence on photoperiod, which had a negative value of \Sexpr{smBPSpMean} (90\% credible interval: \Sexpr{lower_smBPSpMean}, \Sexpr{upper_smBPSpMean}). The negative trait effect estimated from our model produced more negative slopes, however, given the nature of the log transformation of the seed mass data, the slope of the zero trait effect model is positive. This is shown for the large seeded species \textit{Aesculus hippocastanum} in \ref{fig:slopes}, where here too, the negative effect of the seed mass trait produced a more negative slope in the full model compared to the slope of the model with a zero trait effect (Fig.\ref{fig:slopes}). The slope differences of small seeded species, like \textit{Alnus glutinosa}, was considerably small. While the estimated trait effect of photoperiod for all species produced more similar slopes for both the full model and the model with a trait effect set to zero (Fig.\ref{fig:slopes}).
%In line with our our prediction, the high SLA species had earlier budburst dates than the low SLA species.\\

Finally, the relationship between LNC and cue responses, was weaker compared to the relationships with other traits, but we still found a positive association between LNC and cue responses (Fig.\ref{fig:cuetraits}). Chilling was most influenced by LNC (interaction parameter: \Sexpr{lncBCSpMean} :90\% credible interval: \Sexpr{lower_lncBCSpMean}, \Sexpr{upper_lncBCSpMean}), followed by the forcing (interaction parameter: \Sexpr{lncBFSpMean}: 90\% credible interval: \Sexpr{lower_lncBFSpMean}, \Sexpr{upper_lncBFSpMean}), while photoperiod had the smallest influence from LNC (interaction parameter: \Sexpr{lncBPSpMean} :90\% credible interval: \Sexpr{lower_lncBPSpMean}, \Sexpr{upper_lncBPSpMean}). For species like \textit{Alnus glutinosa} that produce leaves with high LNC, the positive trait effect resulted in a more positive slope in the full model relative to the slope of the zero trait effect model. In contrast, species with low LNC like \textit{Quercus ilex} have a much smaller difference in their slopes (Fig.\ref{fig:slopes}). The model estimated trait effect of LNC for photoperiod is relatively negligible, producing no noticeable differences between the full model and the zero trait effect model (Fig.\ref{fig:slopes}). 

\begin{figure}[h!]
    \centering
 \includegraphics[width=\textwidth]{..//..//analyses/traits/figures/slopesConsAcqu_nprior.pdf} 
