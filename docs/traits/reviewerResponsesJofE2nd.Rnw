...
We completely agree with the reviewer that one source of variation that is captured in our study-level estimate is due to site. We have now added this to our list of factors on \lr{R2_295}. While previous metaanalyses have included site as a random effect, by including study instead of site, the study-level estimates account for site-level differences as well as differences in methods or the effects of observer bias. We also added further detail to \lr{R2_295_2} to address how high study-level variation can reduce our ability to interpret interspecific variation. 
...
\begin{quote}
Contrary to our predictions, we found a negligible response to forcing with tree height...
\end{quote}
...
\begin{quote}
Similarly, the lack of a relationship between SLA and temperature was surprising, given that SLA can also mitigate the effects of temperature with small thicker leaves reducing the effects of frost damage in the spring and incluencing leaf size as cold temperatures can reduce leaf growth rates (CITES).
\end{quote}
