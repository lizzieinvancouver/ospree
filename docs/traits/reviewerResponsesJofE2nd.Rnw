...
We can see how this statement would benefit from examples of the different ways in which a single trait can relate to growth form or function. We now elaborate on the different reasons why height and SLA can vary from \lr{R2_314} to \lr{R2_314b}. Additionally, we now address intraspecific variation from \lr{R2_732} to \lr{R2_732b}; while intraspecific variation may be high for other phenological events (such as budset), research to date suggests it is low for woody plant budburst. \\ %emwAug25 -- I would also address intraspecific variation in your reply. See and fix my edits here. 
