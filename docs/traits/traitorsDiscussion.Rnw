...
\item Whether trait effects led to advanced or delayed budburst phenology was generally aligned with well known gradients in traits that span from acquisitive to conservative growth strategies
...
\item Findings also highlight importance of considering variation across studies---often comparable to the differences observed across species. %Oct31 -- since your model is really good at partitioning this. 
\end{enumerate}
...
\item It is necessary to incorporate phenology into species trait syndromes for us infer species responses to climate change.
\begin{enumerate}
...
\item Our results can be used to characterize the trait syndromes of native forest communities and identify whether open temporal niche space is likely to be filled by invasive species.
\item Will also help us predict species potential to shift ranges and invade novel communities as regions of suitable habitat change with climate change.
