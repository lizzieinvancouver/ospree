...
\begin{figure}[h!]
    \centering
 \includegraphics[width= 0.9 \textwidth]{..//..//analyses/traits/figures/traitMap.pdf} %emwApr20 -- nice!
    ...
    \label{fig:map}
\end{figure}
%DLeditsMarch31: need to look into the unexpected budburst and LNC values
\newpage
...
\begin{figure}[h!]
    \centering
 \includegraphics[width= 0.75 \textwidth]{..//..//analyses/traits/figures/metaanalysisDecisionTree.pdf} %emwApr20 -- great new figure! But some typos in the PDF, could someone else read the supp for typos for us? 
    \caption{Our initial aim was to include all species from \citep[analyzed by][]{Morales2024}, however trait data was only available for a subset of these species. Data was cleaned extensively, with each step depicted by a box, and subset to only include traits for adult trees growing under natural conditions for which we had a complete suite of trait values.}
    \label{fig:database}
\end{figure}
...
\begin{figure}[h!]
 ...
 \includegraphics[width= 0.75\textwidth]{..//..//analyses/traits/figures/slopesConsAcqu_4trt.pdf} 
     \caption{ Here we show an example with an acquisitive species, \emph{Alnus incana} shown in blue, and a conservative species, \emph{Quercus rubra} shown in yellow, using the posterior estimates from our height (\textbf{a-c}), SLA (\textbf{d-f}), seed mass (\textbf{g-i}), and LNC (\textbf{j-l}) models to estimate the day of budburst. \textbf{a,d,} The effect of height on budburst timing was smallest in response to forcing, \textbf{b} while the greatest effect of height on budburst was in response to chilling, \textbf{c,} followed by photoperiod. \textbf{e,} The greatest effect of SLA on budburst was only in relation to photoperiod. \textbf{g,h,i,} Seed mass had a negligible effect on our estimates budburst.  \textbf{j,k,l}, But LNC had a considerable effect on budburst timing in relation to each cue. The strength of the relationships can be assessed by comparing the colored bands, with the lighter bands depicting the estimated 50\% uncertainty interval without the effect of traits  ($\mu_{k,g}$ equal to zero) and darker bands the estimated budburst with the effects of traits included. Points represent the raw budburst data.}  
    \label{fig:slopes}
\end{figure}
