...
\begin{figure}[h!]
 ...
 \includegraphics[width= 0.5 \textwidth]{..//..//analyses/traits/figures/metaanalysisDecisionTree.pdf}
    \caption{Trait data was obstained from the try and BIEN trait databases and budburst data from the OSPREE databases, with a focus on the subset of species \citep[analyzed by][]{Moralles2024}. Data was cleaned extensively and subset to only include traits for adult trees growing under natural conditions.}
    \label{fig:database}
\end{figure}
