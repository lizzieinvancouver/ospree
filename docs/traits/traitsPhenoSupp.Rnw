...
\title{Supplementary Material: Woody plant phenological responses are strongly associated with key functional traits}
\date{}
...
\begin{figure}[h!]
    \centering
 \includegraphics[width=\textwidth]{..//..//analyses/traits/figures/slopesConsAcqu_sla_lnc.pdf} 
    \caption{ We expected species with traits associated with acquisitive (e.g, low specific leaf area, SLA, and leaf nitrogen content, LNC)  versus conservative (e.g., high SLA and LNC) growth strategies would have different budburst responses to phenological cues. Our joint model allows traits of species to influence their responses to cues. We show an example here with an acquistive species, \emph{Alnus incana} shown in red, and a conservative species, \emph{Quercus rubra} shown in blue, for SLA (\textbf{a-c}) and LNC (\textbf{d-f}). Our joint model estimated later budburst due to trait effects for both SLA and LNC in response to forcing (\textbf{a, d,}) and chilling (\textbf{b, e}) and for LNC in response to photoperiod (\textbf{f}). Only in response to photoperiod did we estimate the effect of SLA to lead to slightly earlier budburst with longer photoperiods  (\textbf{c}). The coloured bands represent the 50\% uncertainty intervals of the model estimates and points individual trati measurements.} 
...
datasource <- print(xtable(smTab37 , caption="Bibliographic information for trait data sources from both BIEN and Try trait databases.", label = "tableRefs"), include.rownames= F, caption.placement="top", hline.after=c(-1,0),tablular.environment = "longtable", xtable.floating = T)
...
print(xtable(esti.table , caption="Summary of model estimates using measurements of seed mass data for our 37 focal species ($n$ = 281).", label = "tableSM"),include.rownames= T, caption.placement="top", hline.after=c(-1,0))
