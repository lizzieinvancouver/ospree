...
\begin{figure}[h!]
    \centering
 \includegraphics[width= 0.75 \textwidth]{..//..//analyses/traits/figures/PCA_geometricmean.pdf}
...
\begin{figure}[h!]
 ...
 \includegraphics[width= 0.75 \textwidth]{..//..//analyses/traits/figures/metaanalysisDecisionTree.pdf}
    \caption{Trait data was obtained from the try and BIEN trait databases and budburst data from the OSPREE databases, with a focus on the subset of species \citep[analyzed by][]{Morales2024}. Data was cleaned extensively, as depicted by the various boxes, and subset to only include traits for adult trees growing under natural conditions for which we had a complete suite of trait values.}
    \label{fig:database}
\end{figure}
...
\begin{figure}[h!]
 ...
 \includegraphics[width= 0.75\textwidth]{..//..//analyses/traits/figures/slopesConsAcqu_4trt.pdf} 
     \caption{ Here we show an example with an acquisitive species, \emph{Alnus incana} shown in blue, and a conservative species, \emph{Quercus rubra} shown in yellow, using estimates from our height (\textbf{a-c}), SLA (\textbf{d-f}), seed mass (\textbf{g-i}), and LNC (\textbf{j-l}) models to estimate the day of budburst. \textbf{a,d} The effect of height and SLA on budburst timing was smallest in response to forcing, \textbf{b} while the greatest effect of height on budburst was in response to chilling, \textbf{c} followed by photoperiod. \textbf{e} The greatest effect of SLA on budburst was only in relation to photoperiod. \textbf{g,h,i,} A negligible effect was found between seed mass and all three cues.  \textbf{j,k,l}, But LNC had a considerable effect on budburst timing in relation to each cue. The strength of the relationships can be assessed by comparing the colored bands, with the lighter bands depicting the estimated 50\% uncertainty interval (UI) without the effect of traits  ($\mu_{k,g}$ equal to zero) and darker bands the 50\% UI with the effects of traits. Points represent the raw budburst data.}  
    \label{fig:slopes}
\end{figure}
...
\begin{tabular}{rrrrrr}
 ...
 & mean & X5. & X25. & X75. & X95. \\ 
  \hline
$\mu_{grand.trait}$ & 12.70 & 9.40 & 11.40 & 14.00 & 15.8 \\ 
  $\mu_{k,g}$ & 32.10 & 27.80 & 30.30 & 33.90 & 36.5 \\ 
  $\mu_{force}$ & -10.70 & -15.50 & -12.60 & -8.80 & -6.2 \\ 
  $\mu_{chill}$ & -4.10 & -10.80 & -6.80 & -1.40 & 2.6 \\ 
  $\mu_{photo}$ & 1.10 & -2.60 & -0.30 & 2.50 & 4.6 \\ 
  $\beta_{trait.force}$ & 0.20 & -0.20 & 0.00 & 0.30 & 0.5 \\ 
  $\beta_{trait.chill}$ & -0.50 & -1.00 & -0.70 & -0.30 & -0.1 \\ 
  $\beta_{trait.photo}$ & -0.20 & -0.50 & -0.40 & -0.10 & 0.0 \\ 
  $\sigma_{species}$ & 5.90 & 4.80 & 5.40 & 6.40 & 7.3 \\ 
  $\sigma_{study}$ & 7.50 & 5.80 & 6.70 & 8.30 & 9.7 \\ 
  $\sigma_{trait}$ & 5.40 & 5.40 & 5.40 & 5.40 & 5.4 \\ 
  $\sigma_{pheno}$ & 15.10 & 11.80 & 13.80 & 16.40 & 18.6 \\ 
  $\sigma_{force}$ & 5.00 & 3.30 & 4.10 & 5.70 & 7.1 \\ 
  $\sigma_{chill}$ & 8.50 & 5.60 & 7.00 & 9.80 & 12.4 \\ 
  $\sigma_{photo}$ & 3.20 & 2.00 & 2.60 & 3.80 & 4.8 \\ 
  $\sigma_{d}$ & 14.20 & 13.80 & 14.00 & 14.30 & 14.6 \\ 
  \end{tabular}
\end{table}
