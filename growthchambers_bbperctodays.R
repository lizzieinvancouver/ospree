...
#dat<-read.csv("growthchambers_litreview_2016-01-15.csv", header=T)
#head(dat)
#unique(dat$respvar)
#dat$dbb<-NA
...
percbbsites<-dat[which(dat$respvar=="percentbudburst"),]#could add others here...
dim(percbbsites)
...
for(j in 1:length(study)){
  ...
  minperc_bb<-min(dat2$response)#minpercent budburst- hopefully zero
  ...
  if(length(dat3[which(dat3$response==50),]$response.time)==0){
